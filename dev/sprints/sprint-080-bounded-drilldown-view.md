# Sprint 080 — BoundedDrillDownView (Mac).
...
Author the BoundedDrillDownView artboard. Shows source object, hop target, allowed scope, visibility result, record or event summary, hidden fields, reason code. Action: Open allowed hop (`BoundedDrillDown` with `hop_target`). A hop into a hidden field refuses with `bounded_drilldown_denied`. Rule `bounded_drill_down` (`support_user`).
...
- `specs/ui-surface-design/ui-surface-design-spec-v0.3.md §19.7`
...
- `canvas/mac/BoundedDrillDownView.dc.html`
