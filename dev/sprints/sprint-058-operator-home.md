# Sprint 058 — OperatorHome (handheld).

```yaml
---
id: 058
status: closed # [closed 2026-08-26 — OperatorHome authored; D.2 handheld pack opens]
phase: D.2-handheld
...
Author the OperatorHome artboard. Shows the operator's current work and next action. Reads through `readRecordAsCaller(Run)`, `readProjection(RunCloseReadiness)`, `readRecordAsCaller(RunCloseCheck)` for the top blocker. Primary actions: Continue run (routes to RunStepView), Start assigned run (`StartRun`), Scan (routes to Scan tab), View blocker (routes to BlockerView).
...
- `specs/ui-surface-design/ui-surface-design-spec-v0.3.md §11.1, §11.6-OperatorHome`
