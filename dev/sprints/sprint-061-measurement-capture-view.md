# Sprint 061 — MeasurementCaptureView (handheld).

```yaml
---
id: 061
status: closed # [closed 2026-08-26 — MeasurementCaptureView authored; numeric entry, projected result, quality-path link]
phase: D.2-handheld
...
Author the MeasurementCaptureView artboard. Shows measurement name, expected range from the ProcedureStep's DataCollectionField, input value, unit, tool (Instrument.cal_status), result after submit. Primary action: `CaptureMeasurement`. Result states: `not_evaluated`, `pass`, `fail`, `warning`. A `fail` result opens the quality path (registered rule `failed_measurement_has_quality_path` fires at run-close).
...
- `specs/ui-surface-design/ui-surface-design-spec-v0.3.md §11.6-MeasurementCaptureView`
