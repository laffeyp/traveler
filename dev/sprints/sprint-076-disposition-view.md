# Sprint 076 — DispositionView (Mac).
...
Author the DispositionView artboard. Shows disposition kind (`scrap`, `rework`, `repair`, `use_as_is`, `return_to_supplier`), authority requirement (`elevated_disposition_authority` gates `use_as_is` and `repair` on `manufacturing_engineer` or `quality_engineer`; base `disposition_recording` otherwise), affected item or population, reason, resulting allowed path. Action: `RecordDisposition`.
...
- `specs/ui-surface-design/ui-surface-design-spec-v0.3.md §15.4`
...
- `canvas/mac/DispositionView.dc.html`
