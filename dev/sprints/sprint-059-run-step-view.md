# Sprint 059 — RunStepView (handheld).

```yaml
---
id: 059
status: closed # [closed 2026-08-26 — RunStepView authored; the operator's workhorse screen]
phase: D.2-handheld
...
Author the RunStepView artboard. Shows the current step's instruction, required measurements, required inventory, required evidence, current blockers, redline status. Primary actions: Start step (`StartRunStep`), Capture measurement (`CaptureMeasurement`), Attach evidence (composite: `CreateAttachment` + `LinkAttachment` + optional `RouteAttachmentForReview`), Complete step (`CompleteRunStep`), Skip step (`SkipRunStep`), Fail step (`FailRunStep`), Start rework (`StartRunStepRework`). Disabled-state examples on required-measurement-missing, required-installation-missing, attachment-access-denied, presence-asserting-scan.
...
- `specs/ui-surface-design/ui-surface-design-spec-v0.3.md §11.6-RunStepView, §5.4`
