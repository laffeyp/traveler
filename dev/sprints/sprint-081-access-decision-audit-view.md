# Sprint 081 — AccessDecisionAuditView (Mac).
...
Author the AccessDecisionAuditView artboard. Reads the `ACCESS_DECISION_AUDITED` event stream. Per audit row: actor, caller type, target alias, action, decision (allow, summary, denied, hidden_existence), visibility level, reason code, policy version, time, support context. The audit event carries the target alias and reason but no field from the target's data (sprint 049 invariant).
...
- `specs/ui-surface-design/ui-surface-design-spec-v0.3.md §20.3`
...
- `canvas/mac/AccessDecisionAuditView.dc.html`
