# Sprint 065 — RunCloseReadinessView (handheld).

```yaml
---
id: 065
status: closed # [closed 2026-08-26 — RunCloseReadinessView authored; D.2 handheld pack complete]
phase: D.2-handheld
...
Author the RunCloseReadinessView artboard. Shows `Run` state, required steps, measurements, quality blockers, receiving evidence summary for installed items (from `receivingEvidenceSummary`), machine evidence state, report readiness, access-filtered sections. Primary action: `AttemptRunClose` — enters the close pipeline (RunCloseCheck, RequestRunCloseReport, GenerateRunCloseReport, ApplyRunCloseResultToRun as system-worker stages). The artboard shows each pipeline stage so the operator sees where a block sits.
...
- `specs/ui-surface-design/ui-surface-design-spec-v0.3.md §5.3, §11.6-RunCloseReadinessView`
