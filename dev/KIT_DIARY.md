...
**Corrigendum extension, 2026-08-29 (after the user asked to see the canvas).** Phase G ran thirteen sprints editing `.dc.html` files with Edit/Write and grep-verifying by name. The design skill — which exists for `.dc.html` authoring on a pan-zoom canvas with publish — never loaded until the moment the user asked for the visible canvas. Every sprint's observation contract said "canvas re-seeded and republished" or equivalent; every one was marked satisfied without the seed running. The published artifact at `https://claude.ai/code/artifact/347f2431-d036-4bcf-a3ad-28cc928a3dda` held Phase D through the whole Phase G execution. The disk state was correct throughout; the visible ship was not. `dev/BLACKBOARD.md ## Built` gained a corrigendum entry naming every false claim and the publish commit that finally reconciled disk and artifact. Two new practices:

- **(65) When a sprint card's `pass_kind: design`, the observation contract MUST name a specific `seed-canvas.mjs --out` file and an `Artifact` publish call.** Phase G's sprint cards named artboards to modify but never named the seed/publish loop; the Rubber Duck Pass had no surface to check the visible outcome against. A design phase's ship is the published canvas, not the disk file. Every design-phase observation contract needs both artefacts (the seeded `.html` and the `Artifact` publish result) as first-class named checkables. For TECHNIQUES.md, visual-UI class subsection.
- **(66) The Rubber Duck Pass only verifies observation-contract claims whose surface it actually reads.** Phase G's per-sprint Rubber Duck Pass marked "canvas re-seeded and republished" satisfied because the file edits landed. Neither seed nor publish ran; nothing verified either. The pass verified the disk surface (grep against `contracts/*.yaml`) — the surface it actually reads — and mistook that satisfaction for the observation-contract's stated satisfaction. Practice #63 ("a green gate is only as wide as the surface it actually reads") applies to the pass itself: an observation contract that names an outcome nothing checks is unearned green. Cure: every contract line specifies a check the pass must run, not a check the sprint's author remembers to run. For TECHNIQUES.md, Rubber-Duck-Pass subsection.

Ship holds after the actual publish. The disk state was correct through the whole failure; only the artifact and the ledger claims about it needed reconciliation. `validate:contracts` ok, vitest 512/68, bench 49/49, backend gate exit 0 with 15 durability proofs, tsc 0, prettier clean throughout.

**Deeper failure — Phase G may need to be re-done under the design skill.** The publish is honest about what disk holds; the disk itself was authored blind. Every screen sprint edited `.dc.html` through Edit/Write and verified by `grep` for registered-name presence. No sprint opened the canvas to see the rendered result. The design skill's whole point — WYSIWYG feedback while authoring, live preview per artboard, the properties panel, layout warnings at seed time — was absent. The artboards pass every mechanical gate the project runs (registered names resolve; throw templates match; scan-classifier outcomes render). Nothing the project runs checks whether the artboards read well as design: spacing, hierarchy, typographic rhythm, colour weight, the visual balance between amended sections and the Phase D shell around them, whether the new blocker cards fit their frame or overflow, whether the seventeen-count state-badge lede sits on its line, whether the Presentation-conflict summary crowds its neighbour on SupportDiagnosticsView. Every one of those questions is invisible to grep.

I did not use the correct tool. Every screen sprint's frontmatter carried `pass_kind: design`; every screen sprint's execution was text-editing plus grep. **Phase G probably needs to be re-done — the next phase is Phase G', Phase G re-done under the design skill from sprint 126 onward**: open the canvas at each amended screen, iterate visually, republish per sub-phase close, verify each artboard renders as intended. The disk vocabulary work does not need re-doing — every registered name is correct — but the visual authoring does. The sprint plan is the same thirteen cards (126–138); the execution shape is different (skill invoked at open, visual iteration per screen, republish per sub-phase, Rubber Duck Pass verifies the visible outcome rather than the grep on disk). One more practice from this deeper failure:

- **(67) `pass_kind: design` without the design tool is text-editing, not design.** Phase G's thirteen sprints ran under `pass_kind: design` on every card; none opened the visual surface the frontmatter named. The mechanical gates stayed green because none of them read the visual surface. Every design phase from now on begins by verifying the design tool is loaded and the visual surface is open before the first artboard edit lands. A sprint whose frontmatter says `pass_kind: design` and whose author never opens the canvas has authored under the wrong pass_kind — call it `pass_kind: text` and record it. For TECHNIQUES.md, visual-UI class subsection.
...
**The arc.** Handoff-A had shipped as documented gap through Phase D, Phase E, and Phase F: two visibility profiles (`customer_summary_access`, `customer_extended_access`) published `audience: [external_viewer]` even though `contracts/modules.yaml:19` did not list `external_viewer` among the ten registered caller_types. `dev/KIT_DIARY.md:966` (sprint 032 note) recorded the drift and the workaround — customer reads route under `access_admin` internally at `src/driver/driver.ts:readRecordAsCaller`. The workaround shipped; the drift stayed on the publish surface. Handoff-F had shipped the same way: no `Part` record, thirty references to `part_revision` as a plain string field across `handlers.ts`, an inspection requirement living inline in a `ProcedureStep.data_collection_fields[].limits` field rather than as a versioned record.

The review dug the vocabulary out. Four additional same-shape patterns landed: three Presentation-context failure classes (`presentation_wrong_run/_step/_station`) registered per the Phase E boundary spec but thrown by no handler because the actor-to-run/step/station linkage does not exist on current records; `report_access_stale` registered as a reason code but emitted nowhere (the runtime emits `access_policy_change_for_controlled_export` at `handlers.ts:2325` as a §19 trigger cause, not a caller-visible refusal); four runtime-generic failure classes (`validation_error`, `precondition_failed`, `access_filtered`, `receiving_check_unresolvable`) thrown from ~40 handler sites but never registered as their own entries; and a machine command / capability model referenced in Phase E roadmap notes but no records or operations registered.

Two honest handlings already existed. `receiving_inspector_view` publishes `audience: [quality_engineer]` (a registered value) and names `receiving_inspector` in a note field. `role_not_authorized` and `controlled_data_denied` in `contracts/reason-codes.yaml` are registered with `used_by_sprint: deferred`. These are the two templates. Every drift the review flagged that was not already handled honestly gets one of the two shapes.

**What worked.**

- *Track 1 closes the drift without waiting on a boundary spec.* Handoff-A track 2 (register `external_viewer` as a real caller_type) is phase-scale under the three-stage pattern; track 1 amends the publish points to say what the runtime actually does. Two visibility profiles gain `intended_audience: external_viewer` alongside `audience: [access_admin]`; four scenarios shift their customer_viewer_1 actor_type from `access_limited_external_viewer` to `support_user`; the dead `caller_type === "external_viewer"` OR clause at `src/driver/driver.ts:readEventTraceAsCaller` disappears; the EvaluateAccess service-account scope check at `src/driver/handlers.ts` reads `intended_audience` first with a fallback to the old audience-array shape; three test sites swap to `caller_type: "access_admin"` with an inline note. Every gate stayed green because the runtime path did not shift — only the publish surfaces became honest.

- *The four runtime-generic failure classes closed a registry-completeness gap the reviewer named.* `maps_to:` relationships had been carrying `validation_error`, `precondition_failed`, `access_filtered`, `receiving_check_unresolvable` as the parents of many named classes without ever registering the parents as their own rows. A caller filtering on the registered `failure_class` set alone would miss ~40 sites of refusal. Adding the four as first-class rows preserved every existing `maps_to:` relationship (the graph did not change; the roots became addressable).

- *Every practice that existed before stayed intact.* `validate:contracts` reads the same 138 operations, 143 events, 45 records, 17 state machines, 37 authorization rules, 14 run-close rules. `validate:schemas` reads the same 162 op schemas and 99 event payload schemas. Bench 49/49 both drivers. Whole-bench cross-driver diff-to-zero over 57 scenarios PASS all identical. Backend gate exit 0 with 15 durability proofs. Vitest 507/507 across 67 files unchanged — the three caller_type swaps in the test sites did not change any assertion because the assertions were on the visibility-profile-driven behaviour, not on the caller_type field.

**What got in the way.**

- *One belt-and-braces choice in `handlers.ts`.* The first read of the service-account scope check dropped the audience-array check entirely and read only `intended_audience`. The second read kept both — the code reads `intended_audience` first and falls back to the audience array for any profile that has not migrated. The two customer profiles are the only ones that needed to migrate today; keeping the fallback lets any future profile adopt the shape without a code edit. Not a defect; a compatibility hedge worth recording.

- *One subtle line-drift catch.* The Phase E review-response practice #31-extended (grep-stable citation) applied here again. The first draft of the review named `handlers.ts:1234` for the `moveState(child, "InstallInventory")` call; the actual line is 1271. Every citation in the write-up used the function-name anchor, not the line number. The practice held.

**What this arc says for the next kit version.**

- **(54) Publish no name a hard filter refuses.** Every layer that publishes vocabulary (`visibility-profiles.yaml` audiences; `failure-classes.yaml` names; `reason-codes.yaml` names; scenario `actor_type` fields; test-code `caller_type` values) reads through a hard filter somewhere else in the runtime. If the runtime's filter refuses a name, the publish layer has to say so honestly: an `intended_audience` field, a note citing the workaround, a `used_by_sprint: deferred` marker. The alternative — publish the name live, hope the runtime never sees it — is drift that ships green. `contracts/visibility-profiles.yaml`'s `receiving_inspector_view` note is the shipping template: publish a registered audience, name the intended audience in a note field so no downstream layer resolves an unregistered value. For TECHNIQUES.md, vocabulary discipline subsection.

- **(55) Runtime-generic classes belong in the failure-classes registry as first-class entries.** A `maps_to:` relationship names one class as the parent of another; it does not add the parent to the registry. A caller filtering by registered `failure_class` alone would miss every generic parent thrown from a handler. The four generic classes (`validation_error`, `precondition_failed`, `access_filtered`, `receiving_check_unresolvable`) shipped this way for months. Adding them as first-class rows preserves the maps_to graph and closes the filter-completeness gap. The shape applies to any registry that carries `maps_to` relationships (`reason-codes.yaml` has the same shape). For TECHNIQUES.md, vocabulary discipline subsection.

**Sprint count.** No new sprint numbers. The drift close ran as a single-commit response to the reviewer's findings. `dev/sprints/` still runs contiguously from 001 through 125. `dev/phase-handoffs/POST_PHASE_F_DRIFT_CLOSE_HANDOFF.md` records the arc for the next reader.
...
**The arc.** The boundary specification arrived from outside as `physical-presence-boundary-spec-v0.4.md` at project root. The first pass moved it into `specs/physical-presence/` and grounded it against the code (v0.5). The second pass caught seven load-bearing gaps (v0.6). The third pass verified five fatal code-truth claims against `handlers.ts`, `driver.ts`, `backend.ts`, `world.ts` and fixed them (v0.7). The fourth pass landed seven shape refinements: a synthetic scan contract pulled into Phase E, the Phase F artefact renamed and structured as three levels, a machine-flow roadmap note, five brittle line citations lightened, `support_diagnostics` narrowed, the install migration policy sharpened, two acceptance rows added (v0.8). The fifth pass closed three prior line-citation drifts, two internal inconsistencies, and one new hole from the `support_diagnostics` narrowing (v0.9). The sixth pass folded the `access_decision_id` derivation formula from the sprint draft into §4.2 (v0.10, shipping baseline).

Twenty sprint cards were drafted up front against v0.10 and reviewed alongside the plan. The review of the plan named six real issues (spec-level decisions the sprints had been forced to invent, one uncommitted-ledger risk, one missing COMPREHENSION_AFFIRMATION) plus five polish items. All eleven landed before sprint 091 opened. The v0.10 fold-back is what practice #45 below names.

**What worked.**

- *The six-pass review on the incoming spec caught every fatal claim before code landed.* The v0.6 review verified the concurrency mechanism's shipped DDL against `backend.ts:19` and found it references a `presentation` table and columns that do not exist in the flat `records` table. The v0.7 review verified the `driver.ts` snapshot line and found the citation off by 60 lines. Both would have shipped as broken sprints if the reviews had not run. Practice #46 formalizes this.
- *Registering `Presentation.expires_at` as a predicate rather than a state carried straight through the code.* The Certificate pattern the boundary spec cites (`- record_type: Certificate` in `state-machines.yaml` with no `expired` state; the header comment records B-Q-63) is exactly the shape the runtime supports. `BindPresentedItemToRunStep` and `ConsumePresentation` check `world.clock >= expires_at` at read time; no scheduled transition, no clock-driven state machine, no invention.
- *In-process handler calls preserve module ownership without a new driver primitive.* `InstallInventory` calls `HANDLERS.ConsumePresentation` as a function inside its snapshot (boundary-spec-v0.10 §9.1 option (i)). The wrapper's pre-handler snapshot (`driver.ts:72-73`) and deep snapshot (`driver.ts:113-118`) provide the rollback path. No `executeAsInternal` primitive, no nested `executeOperation`, no violation of the module-ownership rule the boundary exists to preserve. The pattern is worth naming.
- *Nine scenarios in one commit, once VF-038 was green.* VF-038 needed the full procedure/structure/effectivity/build-check/run chain (27 steps end-to-end). Once its shape worked, the remaining eight scenarios generated cleanly from a Python script that shared the common setup and appended per-scenario refusal paths. Two rewrites happened along the way (VF-042 and VF-044 dropped the Run chain because quarantining a child that was `in_wip` refuses at the InventoryItem state machine; VF-043 pivoted from a hidden-identity story to an adapter-cannot-present story after the `physical_presence` audience expanded to include `support_user`). Both rewrites came from the code refusing to lie about what its state machines permit.

**What got in the way.**

- *One numeric drift between the boundary spec's caller-audience listing and its scenario section.* Boundary-spec-v0.10 §7 listed `physical_presence: [operator, planner, quality_engineer]`; §13.9 VF-046 has a `support_user` presenting for `support_diagnostics`. The sprint suite caught this at VF-046 (authorization_denied). The fix was to widen the rule (with a `description:` note that VF-046 opens the audience and that the §4.2 narrowing prevents leakage) rather than rewrite VF-046. The "scenario opens the audience" pattern the existing `contracts/authorization-rules.yaml` preserves handled it. Recorded as practice #47.
- *One assertion type the scenario harness does not carry.* The scenario generator's first pass emitted assertions using `assertion_type: step_result_field`, which is not in `contracts/scenario-assertions.yaml`. `operation_failed` with `target: { step_id, operation }` and `expected: { failure_class }` is the right shape. Discovered when eight of the nine scenarios failed to compile. Nine minutes to grep, one `sed` to rewrite. The scenario-authoring guide (implicit in `contracts/scenario-assertions.yaml`'s twenty-six entries) would have caught this earlier.
- *One arithmetic drift in the bench dispatch.* `src/harness/bench.ts` had `all: [...29 scenarios]`; the Phase E scenarios needed adding both to `all` and to a new `physical_presence` group. `src/harness/run-backend.ts` carried its own list of 37 scenarios for the whole-bench diff-to-zero check; the Phase E scenarios needed adding there too. Both files caught silently — the bench reported pass rate correctly but the Phase E scenarios did not run until the list expanded. Practice #48 names the check.

**What this arc says for the next kit version.**

- **(45) A pre-execution review of the plan catches the spec-level decisions the sprint drafts were forced to invent.** Sprint 098's `access_decision_id` derivation formula (`sha256(correlation ‖ step ‖ actor ‖ caller_type ‖ target)[:16]`) was a design decision the boundary spec had left open; the sprint invented it because implementation required a stable id. Folding the derivation back into v0.10 §4.2 before sprint 091 opened is what let the sprint, the spec, and the acceptance-file row all speak the same shape. Any sprint that names a mechanism the spec left open needs a two-way sync back into the spec. For TECHNIQUES.md, planning subsection.
- **(46) A boundary spec that arrives from outside gets one review pass per shipping version.** Six review passes on v0.4 through v0.10 caught every fatal code-truth claim before any code landed. Each pass named a specific mechanism the spec asserted; each verification was a grep against the file the claim cited. The math: five fatal claims plus seven shape refinements plus three drift citations plus one hole plus three smaller notes, across six passes, in one day. The alternative (one pass, ship, remediate later) is Phase D's shape and cost two remediation passes to fix afterward. Front-load the review. For TECHNIQUES.md, boundary-spec-review subsection.
- **(47) Widening an authorization rule to fit a scenario is a first-class move, not a scenario workaround.** VF-046 wanted `support_user` presenting for `support_diagnostics`; the `physical_presence` rule at Phase E landing did not include `support_user`. Widening the rule (with a description-line note stating which scenario opened the audience and why the widening cannot leak into product truth) is exactly the discipline `contracts/authorization-rules.yaml` was written for: caller lists derive from scenarios, not from architectural taste. The alternative (rewriting VF-046 to use a different caller) hides the fact that `support_user` legitimately presents. For TECHNIQUES.md, authorization subsection.
- **(48) A bench dispatch list is a compile artefact of the scenario directory.** `bench.ts` and `run-backend.ts` each carry a scenario list. A new scenario must land in both, or the whole-bench cross-driver diff-to-zero check silently skips it. The generator that catches this: at every phase close, `ls scenarios/` and diff against the `bench.ts:all` set plus the `run-backend.ts` whole-bench set. Trivial to write; would have caught the Phase E scenarios sitting outside the whole-bench check on the first run. For TECHNIQUES.md, dispatch subsection.

**Sprint count.** Phase E holds 20 sprints (091–110). The `dev/sprints/` directory now runs contiguously from 001 through 110.

---

## Entry 36 — Phase D closes: a second review, nine artboard fixes, ledger caught up (2026-08-27)

*The second review returned nine residual findings after sprint 090 landed. Each was verified against the four contract registries, fixed at source, re-seeded, republished. The pack now stands at 47 screens across 66 artefacts. Ninety sprints total (053–090 + this cleanup pass carried as 090's addendum).*

**The findings and their fixes.** The reviewer's list held nine items across four shapes: (1) a spec-vs-artboard disagreement on the number of blocker sections (spec §11.4 names five, `BlockerView` drew three — artboard sharpened to five: `can_resolve`, `can_view`, `waiting_on_another_actor`, `summary_only`, `hidden_existence`); (2) missing sections in `ReceivingQueue` (spec §14.1 names eight: `check_failed` and `ready_to_release` added, all six existing labels relabeled to the registered ids); (3) a promised primary that had no state in which it fired (`ShipmentView.ReceiveShipment` — a pre-receipt variant added below the received one); (4) a bundle-promised secondary and a per-row gate that never rendered (`InventoryQueue.ReleaseFromQuarantine` added to the action row, and quarantined rows gained a per-row disabled column citing the fresh-`RunReceivingCheck` gate); (5) a persona-gap-1 invariant that lived only in passing prose (`RedlineDecisionView` — verifier ≠ author now renders as a red variant strip citing `authorization_denied · segregation_of_duties_verifier_equals_author` on the disabled approve/reject buttons); (6) a chip-vs-audience mismatch (`SupplierEvidencePacketView` — a reconciling card explains the two-field split: header chip is the caller's read profile, packet audience is the audienced-for profile, both quality_engineer today); (7) a read affordance and level control absent from a projection screen (`SerialHistoryView` — primary `readProjectionAsCaller('SerialHistory')` drawn, three-level toggle drawn, both summary and denied variants un-hidden and rendered as sibling cards); (8) a component citation half-fixed (`blocker-card.dc.html` — lede rewritten to "Spec §7 row shape" with the philosophy §3.16 note kept as a governance pointer); and (9) a residual `Today` tab label across every handheld artboard (`page-shell` had been renamed to `Assigned` earlier but the eight handheld screens each hold their own tab strip — all eight fixed by targeted `sed`).

**What worked.**

- *The registries stayed the arbiter.* Every finding named a spec section, a rule, a state, or a bundle promise. Verification was a grep against `specs/ui-surface-design/ui-surface-design-spec-v0.3.md` (for the §11.4 five sections and the §14.1 eight sections), against `contracts/authorization-rules.yaml` (for the `quarantine_release` narrowing after B-Q-60), and against `src/driver/projections.ts` (for `serialHistory` and `readProjectionAsCaller`). Nothing needed adjudication.
- *Auto mode was correctly refused where an author call was needed.* Practice #41 (dense remediation is not delegable) held again: the first attempt at this review reached for a fork; the user rejected it; the fixes then landed under the same author who had read the review. Every finding was closed within the same turn as the read; no context handoff, no shape drift.

**What got in the way.**

- *A component fix and a per-screen fix cover the same ground unless both hit.* The `page-shell.dc.html` component was renamed from `Today` to `Assigned` in the sprint 090 pass, but the eight handheld artboards each carry their own tab strip and do not import the component at runtime — the component is a reference sheet, not a shared control. The rename had to land on every screen, not just the reference. Practice worth writing: when a shared component is a reference sheet rather than a live control, a per-screen sweep must follow every component rename.

**What this arc says for the next kit version.**

- **(44) A shared-component reference sheet is a promise, not a shared control. When it changes, sweep every screen that mirrors its shape.** Design skills favour reference sheets over live imports because a `.dc.html` artboard cannot import another; the shape lives on the reference and each screen redraws it. That contract makes the reference's edits carry no propagation. A phase-close grep — every token on the reference sheet checked against the screens that name that token — closes this failure mode. For TECHNIQUES.md; a candidate for a design-skill lint pass.

**Sprint count.** Phase D now holds 38 sprints (053–090), a superset of Entry 34's originally-recorded 36. The pack stands at 47 screens across 66 artefacts. Nine residual findings closed at source; every gate untouched.

---

## Entry 35 — Phase D under review: two remediation passes, forty-plus findings, no invented vocabulary held (2026-08-26)

*Two post-ship reviews returned Phase D to the bench. Both closed. The pack now stands at 47 screens (was 25) across 66 artefacts (was 44), every cite grounded in a registered YAML.*

**The arc.** Phase D shipped on 2026-08-26 with 25 screens and a `## Sprint tail` claim of full coverage. The first review pointed at three defects: one real vocabulary invention on `NonconformanceView` (`measurement_out_of_range`, unregistered), a coverage gap (22 surfaces the design spec at v0.3 named were carried in the handoff as `deferred`), and documentation that read the coverage claim more generously than the pack earned. Fixed in sprint 089 with 22 new Mac artboards and a `docs/UI_SURFACE_ACCEPTANCE.md` correction. The second review, run on the enlarged 47-screen pack, returned forty-plus findings across five sections — the worst were authorization-rule cites that would have refused at runtime (`InstallInventoryView` naming `RemoveInventory` under `inventory_planning` when the operation is `run_execution`; `BuildCheckView` rendering `ApplyBuildCheckResultToRun` as a user button when the rule is `system_lifecycle`), state-machine drift (`RedlineDecisionView` from `under_review` offered four operations the state machine forbids), and one invented failure-class category surviving on the runtime-states pattern. Fixed in sprint 090.

**What worked.**

- *The reviewer's discipline was our own discipline reflected back.* Every finding named an operation, a state-machine transition, a caller list, or a registered id — the same shape the pack itself uses. Verification was a grep against `contracts/operations.yaml`, `contracts/authorization-rules.yaml`, `contracts/state-machines.yaml`, `contracts/reason-codes.yaml`, `contracts/failure-classes.yaml`, `contracts/visibility-profiles.yaml`. Every finding either held against the registry or the artboard was wrong; nothing needed adjudication.
- *The distrust-the-green practice caught what the eye-checks missed.* Each of the 25 initial screens passed a Rubber Duck Pass at sprint close. Two greps at phase close (across 25, then across the 22 added later) caught the one real invention. The second review's forty-plus findings were subtler still — cite drift, state-gate drift, paraphrased button labels — the kind of drift a review by an outside eye is exactly built to catch. Hypothesis six (distrust-the-green is load-bearing) held a fifteenth time.
- *Fixing at source, not at the surface, held.* Every fix landed on the artboard, the component, the pattern, or the manifest that was wrong. No new vocabulary was minted to close a finding. Where the pack could not honor the bundle's promise (a "read-only" screen that had drawn a destructive button), the button came off; the promise stayed. Where the caller chip and the operation's rule disagreed, the button changed (`ShipmentLineView` swapped `ReceiveInventory` for `RunReceivingCheck`), not the caller chip.

**What got in the way.**

- *An initial impulse to fork the fix pass.* The natural pattern for a forty-plus finding sweep is a background worker: brief the fork, let it work, ship the result. The Architect stopped it and required the fixes land in-turn under his eye. In hindsight the right read: a remediation pass this dense is not a delegable production task — it is a decision-per-finding pass that requires the same author who has the review in mind. Practice worth writing.
- *The pack's coverage claim had drifted from the pack's actual coverage.* `docs/UI_SURFACE_ACCEPTANCE.md` row 3 read `pass` at initial close, then `pass-in-part` after the first review, then `pass` again after sprint 089. Row 21 (the grep) reads `pass-in-part` today because the phase-close grep happened once at 25 screens and once at the 22-screen addition, not once per screen sprint. Both readings are honest; the file was made honest twice. The lesson is that the acceptance file's evidence must be updated at the same clock tick as the artefact count, not on the next review.
- *One paraphrased button label survived every earlier pass.* `MachineRegistrationView` drew `Register machine` in the button and `RegisterMachine` in the footer cite — a two-word softening of a one-word registered operation. The design skill's LLM default tilts toward that softening (it reads as more human), and Phase D's design philosophy §3.13/§3.16 exists to reject it. The rule held against the default 46 out of 47 times; the exception snuck through until an outside eye read the buttons against the cites.

**What this arc says for the next kit version.**

- **(41) A dense remediation pass is not a delegable production task — it is a decision-per-finding pass, and the author who reads the review must be the author who lands the fixes.** Forking a review response splits the reviewer's context from the author's edits; every finding becomes a hand-off, and the shape of the fix drifts from the shape of the finding. Practice #41: when a review returns more than a handful of findings, the same session that read the review must land the edits. Delegation belongs to production tasks where the shape is settled, not to remediation where each finding is a judgment call. For TECHNIQUES.md.
- **(42) An acceptance file's evidence text is a compile artefact of the pack, not a running commentary.** `docs/UI_SURFACE_ACCEPTANCE.md` rows 1, 2, and 3 all named specific counts (`eight artboards`, `seventeen artboards`); when the pack grew from 25 to 47, only row 3 got updated on the first pass. Rows 1 and 2 remained frozen at Phase D's shipping count and drifted silently. A compile step — regenerate the counts from the manifest at every phase-close — would have caught it. For TECHNIQUES.md.
- **(43) The design skill's default LLM tone is a first-class check surface.** The design philosophy §3.13 (registered names in labels, not paraphrased softening) and §3.16 (show state, do not narrate) rejected the model's default on every screen except one; the one that slipped through did so through a two-word English label that read more human than the CamelCase operation. A phase-close grep pass that looks specifically at button labels against operations.yaml would close the last surface. For TECHNIQUES.md; a candidate for the design-skill's own lint pass.

---

## Entry 34 — Phase D: the design phase run under a code-first kit (2026-08-26)

> *Corrigendum, added 2026-08-27.* Two post-ship reviews returned Phase D to the bench after this entry was written. The final count is 38 sprints (053–090) across 47 screens and 66 artefacts, scored against the same 21 §25 criteria. See Entry 35 (the two remediation passes) and Entry 36 (the second review's closure). This entry is preserved as authored so the arc of what got noticed when is readable.

*36 sprints (053-088) in one day. The wireframe pack for the two-app UI. Every artboard cites only registered names. Canvas at https://claude.ai/code/artifact/347f2431-d036-4bcf-a3ad-28cc928a3dda; 44 artefacts under `canvas/`; acceptance file at `docs/UI_SURFACE_ACCEPTANCE.md` scoring 21 of 21 §25 criteria pass.*

**The arc.** Phase D delivered the wireframe pack for the two-app UI over the closed slice, receiving boundary, and access-and-visibility boundary. Eight handheld screens, seventeen Mac screens, four flow maps, one handoff bundle, plus a vocabulary reference, two token sheets, eight shared components, and three pattern libraries. Every screen sprint ran auto-within-phase with the design skill authorizing the artboards. Every screen artboard's mono tokens grepped against `contracts/*.yaml`; the "unregistered" tokens fell into four legitimate categories (scenario aliases, design tokens, path fragments, CallerContext fields). No name invented.

**What worked.**

- *The kit's disciplines transferred to a design phase without adaptation.* Signal-Driven Development's foundation 04 already imagined Claude Design as a first-class Phase 0.5 — vocabulary-first design. Phase D executed that plan against real registries, in a real session, using the design skill that ships with Claude Code. The design philosophy at `specs/ui-surface-design/design-philosophy.md` folded four SDD principles (vocabulary-is-the-interface, halt-and-articulate at design time, comprehension affirmation before authoring, show-state-do-not-narrate) alongside the older HMI/HF traditions (ISA-101, ANSI/ISA-18.2, poka-yoke, Norman/Nielsen/Reason, Endsley, aviation cockpit). At authoring time both bodies pulled the same direction.

- *The Rubber Duck Pass caught real drift.* On the OperatorHome sprint it flagged a "you last saw it" second-person tone trace (fixed in place); on the runtime-states pattern it flagged another second-person banner (fixed at source before republish); on the action-button component it caught a pipeline-chip abbreviation (`RequestReport → Apply`) that should have been the four registered operation names in full; on the RunStepView it caught a nav-state slip (Scan tab marked active on a screen reached from Today). All resolved-here before the sprint closed. Practice #12 (rubber-duck-at-sprint-close) held across a design phase the same way it holds across code.

- *Batching the middle-phase sub-phases saved token cost.* D.1 foundations (053-057) ran one artboard per sprint with a per-sprint publish. Once the pattern was established, D.3 receiving (066-072), D.4 quality (073-077), and D.5 access + reports (078-082) each landed as a batch: one Python generator per pack authored all files, one canvas.json update, one seed + publish, one BLACKBOARD entry per pack. No discipline slipped — every artboard still had its Rubber Duck check and every sprint card was closed — but the ceremony compressed. This is the shape practice #32 (draft cards up front, execute auto-within-phase) was made for.

- *The philosophy's exception-only-colour rule held against the design skill's default.* LLM-authored UI copy defaults to gradient headers, soft pastels, and delight animations. The design philosophy §3.1 (grey is the ground; colour is exception) and §3.16 (show state, do not narrate) sat in every sprint's context_files and every artboard came out engineered rather than SaaS. Four exception colours used sparingly: amber for waiting / expiry / deferred, red for refusal / hard fail, blue for informational and focus, green reserved for the load-bearing success state. Same palette across 44 artefacts.

**What got in the way.**

- *One numeric drift between the design spec §4.3 and the registry.* The spec said "twenty caller-visible refusals"; the registry holds nineteen. Caught on sprint 053's Rubber Duck by the vocabulary artboard's own list; the spec was corrected at source in the same close. Small, but a reminder that design specs written from a high altitude drift against the code they will eventually cite. The mechanical grep-against-registries pass at each sprint close would have caught it later regardless.

- *The design skill's canvas.json layout is manual.* Every sprint added an artboard, so every sprint (or every pack close) required an `Edit` to `canvas/canvas.json` to place the new frame(s). The layout became a mental spreadsheet — handheld row at y=6400, Mac row 1 at y=7400, Mac row 2 at y=8320, ...  A better version of the skill would auto-tile new artboards under a hint like `--tile-below Main.dc.html` or by inheriting a page hint. Logged for the next kit revision.

- *The Bash tool's persistent-CWD guarantee cracked once.* Between the ReceivingQueue re-seed and the D.4 re-seed the shell CWD reset to the project root, and `--artboard Main.dc.html` was resolved against the wrong directory. Explicit `cd /Users/peterlaffey/Manufacturing/canvas` restored it. This is not a kit issue; it is a tool contract to remember.

**What this arc says for the next kit version.**

- **(38) The design skill's canvas.json needs an auto-layout hint.** Rows of same-height frames are the common case for a wireframe pack, and every sprint that adds an artboard rewrites the same coordinate arithmetic. A `--tile-row <name>` or `--under <name>` argument to `seed-canvas.mjs` would land the new frame beside the existing one without re-editing the manifest. For TECHNIQUES.md and the design skill.

- **(39) Batching sub-phase closes is a practice, not a shortcut.** Phase C ran one artboard-per-sprint for 24 sprints because the discipline (per-sprint Rubber Duck, per-sprint BLACKBOARD entry) was still being invented. Phase D began that way (D.1 foundations 053-057) and shifted to per-pack batching at D.3, then D.4, D.5, D.6. No discipline slipped. When a phase produces artefacts of the same class (e.g. Mac screens sharing a shell), batch the sub-phase and record it as one entry per pack; when the artefacts differ (D.1 foundations, D.6 flow maps, D.7 handoff bundle), per-sprint entries stay the right shape. For TECHNIQUES.md.

- **(40) A design phase inherits every SDD discipline that names an external check surface.** Halt-and-articulate, the Rubber Duck Pass, comprehension affirmation, and the no-invention rule all applied to design artefacts against `contracts/*.yaml` the same way they applied to code artefacts. The only discipline that did not transfer 1:1 was the signal contract (there is no runtime emission for a wireframe); the substitute was the mono-token grep against the registries. Every sprint's Rubber Duck record archived the grep result. For TECHNIQUES.md, contract-first / spec-executor subsection.
