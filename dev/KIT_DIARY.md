...
A designer opening the published canvas may find real defects the disk gates cannot catch. **The likely next move is Phase G' — a re-do under the design skill from sprint 126 onward**: open the canvas at each amended screen, iterate visually, republish per sub-phase close, verify each artboard renders as intended. The disk vocabulary work does not need re-doing — every registered name is correct — but the visual authoring likely does. Re-visit condition: Architect review of the published canvas → either "hold, ship as is" or "open Phase G' against a defect list." One more practice from this deeper failure:
