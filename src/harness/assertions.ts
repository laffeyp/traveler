...
export interface Driver {
  ...
  executeOperation(op: string, input: any, caller: string, stepId: string, idempotencyKey?: string, actorId?: string): any;
...
function recEq(rec: any, key: string, val: any): boolean {
  ...
  const got = Object.hasOwn(rec.fields, key) ? rec.fields[key] : (Object.hasOwn(rec, key) ? rec[key] : undefined);
  return got === val;
}
...
export const EVALUATORS: Record<string, Evaluator> = {
  ...
  operation_succeeded(a, { stepResults }) {
    const t = a.target ?? {}, e = a.expected ?? {};
    const r = stepResults.get(t.step_id);
    const ok = !!r && r.succeeded === (e.succeeded ?? true);
    return { ok, msg: ok ? "" : `step ${t.step_id} (${t.operation}) succeeded=${r?.succeeded} failureClass=${r?.failureClass} ${JSON.stringify(r?.output ?? {})}` };
  },
  operation_output_contains(a, { stepResults }) {
    ...
    const t = a.target ?? {}, e = a.expected ?? {};
    if (Object.keys(e).length === 0) return { ok: false, msg: `operation_output_contains ${a.assertion_id} has empty expected` };
    const r = stepResults.get(t.step_id);
    const out = r?.output ?? {};
    const ok = !!r && Object.entries(e).every(([k, v]) => out[k] === v);
    return { ok, msg: ok ? "" : `step ${t.step_id} (${t.operation}) output ${JSON.stringify(out)} does not contain ${JSON.stringify(e)}` };
  },
  operation_failed(a, { stepResults }) {
    const t = a.target ?? {}, e = a.expected ?? {};
    const r = stepResults.get(t.step_id);
    const ok = !!r && r.succeeded === false && (!e.failure_class || r.failureClass === e.failure_class);
    return { ok, msg: ok ? "" : `step ${t.step_id} expected to FAIL(${e.failure_class ?? "any"}) but succeeded=${r?.succeeded} failureClass=${r?.failureClass}` };
  },
  ...
  event_emitted(a, { events }) {
    ...
    if (t.producer_operation) matches = matches.filter((ev) => ev.producer_operation === t.producer_operation);
    let ok: boolean;
    if (e.count !== undefined) ok = matches.length === e.count;
    else ok = matches.length >= (e.at_least ?? 1);
    return { ok, msg: ok ? "" : `event ${t.event_type}${t.producer_operation ? " by " + t.producer_operation : ""}${t.step_id ? " @step " + t.step_id : ""}: got ${matches.length}, expected ${e.count !== undefined ? "==" + e.count : ">=" + (e.at_least ?? 1)}` };
  },
  event_payload_contains(a, { events, driver }) {
    ...
    const t = a.target ?? {}, e = a.expected ?? {};
    if (Object.keys(e).length === 0) return { ok: false, msg: `event_payload_contains ${a.assertion_id} has empty expected payload` };
    let matches = events.filter((ev) => ev.type === t.event_type);
    if (t.producer_operation) matches = matches.filter((ev) => ev.producer_operation === t.producer_operation);
    const ok = matches.some((ev) => Object.entries(e).every(([k, v]) => {
    ...
    return { ok, msg: ok ? "" : `no ${t.event_type} event with payload containing ${JSON.stringify(e)} (${matches.length} candidates)` };
  },
  event_sequence_matches(a, { events }) {
    ...
    if (expected.length === 0) return { ok: false, msg: `event_sequence_matches ${a.assertion_id} has empty expected sequence` };
    ...
    return { ok, msg: ok ? "" : `event sequence mismatch: expected ${JSON.stringify(expected)} but observed(filtered) ${JSON.stringify(filtered)}` };
  },
  event_not_emitted(a, { events }) {
    const t = a.target ?? {};
    let matches = events.filter((ev) => ev.type === t.event_type);
    if (t.producer_operation) matches = matches.filter((ev) => ev.producer_operation === t.producer_operation);
    if (t.after_step !== undefined) matches = matches.filter((ev) => Number(ev.step_id) > Number(t.after_step));
    if (t.before_step !== undefined) matches = matches.filter((ev) => Number(ev.step_id) < Number(t.before_step));
    const ok = matches.length === 0;
    const win = t.after_step !== undefined || t.before_step !== undefined ? ` in window (${t.after_step ?? ""},${t.before_step ?? ""})` : "";
    return { ok, msg: ok ? "" : `event ${t.event_type}${t.producer_operation ? " by " + t.producer_operation : ""}${win} was emitted ${matches.length}x (must not be)` };
  },
  ...
  record_state(a, { driver, checkpoints }) {
    ...
    if (t.checkpoint) { const k = String(t.checkpoint).replace("after_step_", ""); stateAt = checkpoints.get(k)?.get(t.alias); }
    else stateAt = driver.readRecord(t.alias)?.state;
    const ok = stateAt === want;
    return { ok, msg: ok ? "" : `record ${t.alias} state=${stateAt}${t.checkpoint ? " @" + t.checkpoint : ""}, expected ${want}` };
  },
  record_field_equals(a, { driver }) {
    const t = a.target ?? {}, e = a.expected ?? {};
    const r = driver.readRecord(t.alias);
    const ok = !!r && Object.entries(e).every(([k, v]) => {
    ...
    return { ok, msg: ok ? "" : `record ${t.alias} fields mismatch: state=${r?.state} fields=${JSON.stringify(r?.fields ?? {})} expected ${JSON.stringify(e)}` };
  },
  ...
  projection_contains(a, { driver }) {
    ...
    if (t.projection === "AsBuiltProjection") {
      ...
      return { ok, msg: ok ? "" : `AsBuilt(${t.key_alias}) children=${JSON.stringify(proj.children.map((c: any) => c.child_alias))} missing ${e.child_alias}` };
    }
    if (t.projection === "SerialHistory") {
      const proj = driver.readProjection("SerialHistory", t.key_serial);
      const exp = e.event_types ?? [];
      if (exp.length === 0) return { ok: false, msg: `SerialHistory assertion ${a.assertion_id} has empty expected.event_types` };
      const missing = exp.filter((x: string) => !proj.event_types.includes(x));
      const ok = missing.length === 0;
      return { ok, msg: ok ? "" : `SerialHistory(${t.key_serial}) missing event types ${JSON.stringify(missing)} (have ${proj.event_types.length})` };
    }
  ...
  projection_not_contains(a, { driver }) {
    ...
    if (t.projection === "SerialHistory") {
      ...
      return { ok, msg: ok ? "" : `SerialHistory(${t.key_serial}) unexpectedly contains ${JSON.stringify(present)}` };
    }
  ...
  access_denied(a, { driver }) {
    ...
    const ok = proj.view === "denied" && (proj.entries ?? []).length === 0 && (proj.visible_detail ?? []).length === 0;
    return { ok, msg: ok ? "" : `access_denied ${a.assertion_id}: view=${proj.view} entries=${(proj.entries ?? []).length}` };
  },
  bounded_drill_down_filtered(a, { stepResults }) {
    ...
    if (e.hidden) {
      ...
      return { ok, msg: ok ? "" : `BoundedDrillDown hidden=${JSON.stringify(out.hidden)} visible=${JSON.stringify(out.visible)} missing ${JSON.stringify(missing)} leaked ${JSON.stringify(leaked)}` };
    }
  ...
  report_field_equals(a, { driver }) {
    ...
    if (!rec) return { ok: false, msg: `report_field_equals ${a.assertion_id}: report ${t.alias} does not exist` };
    if (rec.record_type !== "GeneratedReport") return { ok: false, msg: `report_field_equals ${a.assertion_id}: ${t.alias} is ${rec.record_type}, not GeneratedReport` };
    if (!t.path || !("value" in e)) return { ok: false, msg: `report_field_equals ${a.assertion_id}: requires target.path and expected.value` };
    let cur: any = rec.fields, missing = false;
    for (const seg of String(t.path).split(".")) {
      if (cur && typeof cur === "object" && Object.hasOwn(cur, seg)) cur = cur[seg]; // own-property only (prototype-safe)
      else { missing = true; break; }
    }
    const ok = !missing && cur === e.value;
    return { ok, msg: ok ? "" : missing ? `report_field_equals ${a.assertion_id}: path ${t.path} not present on ${t.alias}` : `report_field_equals ${a.assertion_id}: ${t.path} = ${JSON.stringify(cur)}, expected ${JSON.stringify(e.value)}` };
  },
  ...
  grammar_gap_created(a, { driver, events }) {
    ...
    const e = a.expected ?? {};
    const gaps = [...(driver.world.records?.values() ?? [])].filter((r: any) => r.record_type === "GrammarGap");
    const created = events.filter((ev) => ev.type === "GRAMMAR_GAP_CREATED");
    const problems: string[] = [];
    if (e.count !== undefined) { if (created.length !== e.count) problems.push(`GRAMMAR_GAP_CREATED count ${created.length} != ${e.count}`); }
    else if (created.length < 1) problems.push(`no GRAMMAR_GAP_CREATED emitted`);
    if (gaps.length < 1) problems.push(`no GrammarGap record`);
    if (e.reason && !gaps.some((g: any) => g.fields.reason === e.reason)) problems.push(`no GrammarGap with reason '${e.reason}'`);
...
function accessFullOrSummary(a: any, { driver }: AssertContext): { ok: boolean; msg: string } {
  ...
  const problems: string[] = [];
  for (const m of e.visible_detail_contains ?? []) if (!vd.includes(m)) problems.push(`missing controlled detail ${m} under ${t.access_profile ?? "full"}`);
  ...
  const fullVd = (e.visible_detail_not_contains ?? []).length ? (driver.readProjection("SerialHistory", t.key_serial).visible_detail ?? []) : [];
  for (const m of e.visible_detail_not_contains ?? []) {
    if (!fullVd.includes(m)) problems.push(`visible_detail_not_contains ${m} is vacuous — ${m} is not exposed under full either`);
    else if (vd.includes(m)) problems.push(`leaked controlled detail ${m} under ${t.access_profile ?? "full"}`);
  }
  for (const x of e.event_types_contain ?? []) if (!et.includes(x)) problems.push(`missing summary-safe event ${x}`);
  if ((e.visible_detail_contains ?? []).length === 0 && (e.visible_detail_not_contains ?? []).length === 0 && (e.event_types_contain ?? []).length === 0) problems.push(`${a.assertion_id} asserts nothing (empty expected)`);
