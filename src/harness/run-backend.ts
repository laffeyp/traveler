...
console.log(`backend run: VF-003 [backend/node:sqlite]  status=${run.result.status}  assertions=${run.result.assertions.passed}/${run.result.assertions.total}`);
...
console.log(`  fresh-instance DURABLE re-eval (persisted-state assertions; checkpoints replayed from event log): ${dur.passed}/${dur.total} passed  (${dur.excluded} operation-outcome assertions excluded — behavior, not persisted state)`);
console.log(`  fresh run_001.state (from disk): ${fresh.readRecord("run_001")?.state}`);
console.log(`  fresh run_001 @after_step_047 (replayed from log): ${reconstructed.get("047")?.get("run_001")}`);
for (const f of run.result.failed_assertions.slice(0, 20)) console.log(`    RUN FAIL ${f.assertion_id}: ${f.message}`);
for (const f of dur.failures.slice(0, 20)) console.log(`    DURABLE FAIL ${f.assertion_id}: ${f.message}`);
...
for (const id of EQUIV_SCENARIOS) {
  ...
  const mn = normTrace(mem.driver.readEventTrace()), bn = normTrace(bk.driver.readEventTrace());
  if (JSON.stringify(mn) !== JSON.stringify(bn)) {
    equivOk = false; equivFailures.push(id);
    let fd = -1;
    for (let k = 0; k < Math.max(mn.length, bn.length); k++) if (JSON.stringify(mn[k]) !== JSON.stringify(bn[k])) { fd = k; break; }
    console.log(`    DIVERGE ${id} at event ${fd}: mem=${JSON.stringify(mn[fd])}  back=${JSON.stringify(bn[fd])}`);
  }
}
console.log(`  cross-driver diff-to-zero over the whole bench (${EQUIV_SCENARIOS.length} scenarios): ${equivOk ? "PASS (all identical)" : "FAIL: " + equivFailures.join(",")}`);
...
console.log(`backend run: VF-006 [backend/node:sqlite]  status=${run6.result.status}  assertions=${run6.result.assertions.passed}/${run6.result.assertions.total}`);
...
const blockerPersisted = fresh6.readEventTrace().some((e: any) => e.type === "BUILD_BLOCKER_CREATED" && e.payload?.blocker === "missing_bom_inventory:gasket_rev_b");
console.log(`  VF-006 fresh-instance DURABLE re-eval: ${dur6.passed}/${dur6.total} passed  (${dur6.excluded} outcome assertions excluded)`);
console.log(`  fresh run_001.state=${fresh6.readRecord("run_001")?.state}  build_check_001.state=${fresh6.readRecord("build_check_001")?.state}  blocker_persisted=${blockerPersisted}`);
for (const f of dur6.failures.slice(0, 20)) console.log(`    VF-006 DURABLE FAIL ${f.assertion_id}: ${f.message}`);
const ok6 =
...
console.log(`backend run: VF-008 [backend/node:sqlite]  status=${run8.result.status}  assertions=${run8.result.assertions.passed}/${run8.result.assertions.total}`);
...
console.log(`  VF-008 fresh-instance DURABLE re-eval: ${dur8.passed}/${dur8.total} passed  (${dur8.excluded} outcome assertions excluded)`);
console.log(`  fresh snapshot.procedure_version=${snapPv}  fresh resolution_002.selected_procedure_version=${res2Pv}`);
for (const f of dur8.failures.slice(0, 20)) console.log(`    VF-008 DURABLE FAIL ${f.assertion_id}: ${f.message}`);
const ok8 =
...
console.log(`backend run: VF-009 [backend/node:sqlite]  status=${run9.result.status}  assertions=${run9.result.assertions.passed}/${run9.result.assertions.total}`);
...
console.log(`  fresh-from-disk views: full=${full9.view}(detail ${JSON.stringify(full9.visible_detail)})  summary=${summary9.view}(detail ${JSON.stringify(summary9.visible_detail)})  unresolvable=${denied9.view}`);
const ok9 =
  run9.result.status === "passed" &&
  full9.view === "full" && full9.visible_detail.includes("controlled_machine_evidence_payload") && // full still exposes controlled detail
  summary9.view === "summary" && !summary9.visible_detail.includes("controlled_machine_evidence_payload") && summary9.entries.length > 0 && // summary still FILTERED (not full, not emptied)
  denied9.view === "denied" && denied9.entries.length === 0; // unresolvable profile still FAILS CLOSED after reload
...
console.log(`backend run: VF-013 [backend/node:sqlite]  status=${run13.result.status}  assertions=${run13.result.assertions.passed}/${run13.result.assertions.total}`);
...
console.log(`  VF-013 fresh-instance DURABLE re-eval: ${dur13.passed}/${dur13.total} passed  (${dur13.excluded} outcome assertions excluded)`);
console.log(`  fresh redline_001@after_step_020 (replayed from alias-carrying events)=${redlineAt020}  final=${fresh13.readRecord("redline_001")?.state}`);
for (const f of dur13.failures.slice(0, 20)) console.log(`    VF-013 DURABLE FAIL ${f.assertion_id}: ${f.message}`);
const ok13 =
...
console.log(`backend run: VF-015 [backend/node:sqlite]  status=${run15.result.status}  assertions=${run15.result.assertions.passed}/${run15.result.assertions.total}`);
const fresh15 = new BackendProductDriver(DB15);
const gaps15 = [...fresh15.world.records.values()].filter((r: any) => r.record_type === "GrammarGap");
console.log(`  fresh-from-disk: GrammarGaps=${gaps15.length} reason=${gaps15[0]?.fields?.reason}  bad_evidence.state=${fresh15.readRecord("bad_evidence_001")?.state}  good=${fresh15.readRecord("good_evidence_001")?.state}`);
const ok15 =
  run15.result.status === "passed" &&
  gaps15.length === 1 && gaps15[0].fields.reason === "unsupported_payload_type" && // the gap persisted with its reason
...
const firstWrite = bwrite.executeOperation("CreateInventoryItem", { inventory_alias: "wbi", serial_number: "WBI-1", part_revision: "p" }, "planner", "s1", "WBI-KEY");
const bcold = new BackendProductDriver(DBW); // fresh instance reconstructs the seen-keys set from disk
const coldRetry = bcold.executeOperation("CreateInventoryItem", { inventory_alias: "wbi2", serial_number: "WBI-1", part_revision: "p" }, "planner", "s2", "WBI-KEY");
const invRows = (bcold as any).countPersisted().records;
console.log(`  write-boundary: first=${firstWrite.succeeded}  COLD-reload re-attempt=${coldRetry.succeeded}/${coldRetry.failureClass}  persisted records=${invRows}`);
const okW =
  firstWrite.succeeded === true &&
  coldRetry.succeeded === false && coldRetry.failureClass === "idempotency_conflict"; // dedup survived the cold reload
...
bidc.executeOperation("CaptureCertificate", { certificate_alias: "certA", cert_type: "mill_cert", serial_or_lot: "LOT-A" }, "planner", "s1");
const idA = bidc.readRecord("certA")?.id;
const bcold2 = new BackendProductDriver(DBI); // reload from disk
bcold2.executeOperation("CaptureCertificate", { certificate_alias: "certB", cert_type: "cofc", serial_or_lot: "LOT-B" }, "planner", "s2");
...
console.log(`  record-id counter reload proof: certA=${idA} certB=${idB} A-intact=${aIntact} -> ${okIdc ? "PASS" : "FAIL"}`);
...
console.log(`backend run: VF-012 [backend/node:sqlite]  status=${run12.result.status}  assertions=${run12.result.assertions.passed}/${run12.result.assertions.total}`);
...
console.log(`  fresh-from-disk: t0.state=${t0?.state}(snapshot ${snap0}, generated_at ${t0?.fields?.generated_at})  t1.state=${t1?.state}(snapshot ${snap1})  REPORT_SUPERSEDED=${sup12.length}/${sup12[0]?.payload?.trigger}`);
const ok12 =
  ...
  sup12.length === 1 && sup12[0].payload?.trigger === "access_policy_change_for_controlled_export"; // the supersede trigger persisted
...
const getR = freshR.executeOperation("GetReport", { report_alias: "run_close_report_001" }, "system_worker", "s", undefined, "w").output;
