...
export function executeScenario(id: string, driver: Driver): Execution {
  ...
  const callerOf = new Map<string, string>((scn.actors ?? []).map((a: any) => [a.actor_id, a.product_caller_type]));
  ...
  driver.world.partRevisions = new Map((scn.world?.part_revisions ?? []).map((p: any) => [p.alias, { part_number: p.part_number, revision: p.revision }]));
  ...
  for (const s of scn.steps ?? []) {
    ...
    const res = driver.executeOperation(s.operation, s.input ?? {}, callerOf.get(s.actor) ?? "unknown", s.step_id, s.idempotency_key, s.actor);
 ...
 */
export function evaluateAssertions(compiled: any, driver: Driver, stepResults: Map<string, any>, checkpoints: Map<string, Map<string, string>>) {
  ...
  for (const a of compiled.compiled_assertions) {
    ...
    try {
      ...
      const evaluate = Object.hasOwn(EVALUATORS, a.assertion_type) ? EVALUATORS[a.assertion_type] : undefined;
      if (!evaluate) msg = `unknown assertion_type ${a.assertion_type}`;
      else ({ ok, msg } = evaluate(a, ctx));
    } catch (err: any) { ok = false; msg = `assertion threw: ${err.message}`; }
    if (ok) passed++; else failures.push({ assertion_id: a.assertion_id, message: msg });
  }
...
const NON_DURABLE = new Set(["operation_succeeded", "operation_failed", "bounded_drill_down_filtered"]);
export function evaluateDurable(compiled: any, driver: Driver, reconstructedCheckpoints: Map<string, Map<string, string>>) {
  const durable = { compiled_assertions: compiled.compiled_assertions.filter((a: any) => !NON_DURABLE.has(a.assertion_type)) };
  const ev = evaluateAssertions(durable, driver, new Map(), reconstructedCheckpoints);
  return { ...ev, excluded: compiled.compiled_assertions.length - durable.compiled_assertions.length };
}
...
export function runIdempotencyReplay(driver: Driver, scn: any): { assertion_id: string; message: string }[] {
  const failures: { assertion_id: string; message: string }[] = [];
  ...
  for (const chk of scn.idempotency_replay_checks ?? []) {
    ...
    if (!step) { failures.push({ assertion_id: chk.check_id, message: `replay: step ${chk.step_id} not found` }); continue; }
    ...
    const res = driver.executeOperation(step.operation, step.input ?? {}, "replay", step.step_id, key, step.actor);
    const dRecs = driver.world.records.size - beforeRecs;
    const dEvents = driver.world.events.length - beforeEvents;
    const problems: string[] = [];
    if (dRecs !== 0 || dEvents !== 0) problems.push(`created ${dRecs} records + ${dEvents} events (must be 0 — duplicate facts)`);
    if (!(res.succeeded === true || res.failureClass === "idempotency_conflict")) problems.push(`returned succeeded=${res.succeeded} failureClass=${res.failureClass}`);
    if (problems.length) failures.push({ assertion_id: chk.check_id, message: `replay of ${step.operation} (key ${key}): ${problems.join("; ")}` });
  }
  return failures;
}

/** Execute + evaluate on a given driver, write trace artifacts, return the ScenarioResult. */
export function runScenarioOnDriver(id: string, driver: Driver, driverName = "in_memory", traceSubdir = id): { result: ScenarioResult; driver: Driver; ex: Execution } {
  const ex = executeScenario(id, driver);
  const base = { scenario_id: id, scenario_version: String(ex.scn.scenario_version ?? "?"), registry_version: ex.compiled.registry_version, product_build: "build_001", driver: driverName };
  if (ex.compiled.status !== "passed") {
    return { result: { ...base, status: "failed", compilation_status: ex.compiled.status, steps_executed: 0, assertions: { total: 0, passed: 0, failed: 0 }, failed_assertions: [{ assertion_id: "compilation", message: `scenario did not compile (${ex.compiled.errors.length} errors / ${ex.compiled.contract_gaps.length} gaps)` }] }, driver, ex };
  }
  ...
  const result: ScenarioResult = { ...base, status, compilation_status: ex.compiled.status, steps_executed: ex.executed, assertions: { total, passed, failed: allFailures.length }, failed_assertions: allFailures };
  const outDir = join(ROOT, `artifacts/traces/${traceSubdir}`);
  mkdirSync(outDir, { recursive: true });
  writeFileSync(join(outDir, "operation_trace.json"), JSON.stringify([...ex.stepResults.entries()].map(([step_id, r]) => ({ step_id, ...r })), null, 2) + "\n");
  writeFileSync(join(outDir, "event_trace.json"), JSON.stringify(driver.readEventTrace(), null, 2) + "\n");
  writeFileSync(join(outDir, "scenario_result.json"), JSON.stringify(result, null, 2) + "\n");
  return { result, driver, ex };
}

export function runScenarioWithDriver(id: string): { result: ScenarioResult; driver: InMemoryProductDriver } {
...
export function runScenario(id: string): ScenarioResult { return runScenarioWithDriver(id).result; }
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  console.log(`  assertions: ${r.assertions.passed}/${r.assertions.total} passed, ${r.assertions.failed} failed`);
  console.log(`  status: ${r.status}`);
  for (const f of r.failed_assertions.slice(0, 40)) console.log(`    FAIL ${f.assertion_id}: ${f.message}`);
