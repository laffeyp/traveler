# Cell-Native Direction Change v0.10

## Direction baseline with pivot taxonomy named

Written 2026-09-25.

This document supersedes `cell-native-direction-change-v0.9.md` (prior candidate baseline). It adds §3a naming the pivot's Ries-taxonomy position and its migration pattern; every other section stands unchanged from v0.9. The change is folded into this doc rather than an external note because the pivot's type is a direction-level fact — it belongs where the direction lives.
...
| v0.9 | baseline | Closed both review-agent findings at source. §15 Machine-family line numbers corrected to `contracts/records.yaml:37/:38/:39`; §5 `factory_node` description corrected to name the shipped locus at `src/driver/visibility.ts:34`. Grep-verified. |
| v0.10 (this) | baseline | Adds §3a naming the pivot's Ries-taxonomy position (Zoom-In + Customer Segment) and its migration pattern (strangler fig on the product-surface layer). Aligns with `dev/adrs/ADR-001-cell-native-pivot.md`, accepted 2026-09-25. Every other section unchanged from v0.9. |
...
# 3a. What kind of pivot this is

This is a **Zoom-In + Customer Segment** pivot in the Ries taxonomy (Eric Ries, *The Lean Startup*, 2011).
...
**Customer Segment**: the first-product customer shifts from broad-factory operators, planners, and quality engineers (as modeled by Phase D's 47-screen surface) to a small repair-cell team of 2–8 workers with shift handoff, shared inventory, and cell-scoped release gates.

The pivot is explicitly **not**:

- A Technology pivot — §2 preserves the runtime, contracts, laws, scenario discipline, visibility/access rules, physical presence, scan classification, bench discipline, phase handoff discipline.
...
Naming the type matters because each pivot type has different discipline. A Zoom-In + Segment pivot demands **core preservation plus surface redraw**, which is exactly what §2 asserts and what G2 attempts.

## Migration pattern

The pivot uses **strangler fig** (Martin Fowler, *StranglerFigApplication*) on the product-surface layer. The old broad-factory UI (canvas Phase D + Phase G overlay) is preserved on disk as reference-only. The new cell-native surface, authored under `docs/phases/g2/` and forward through Phase H reset, strangles the old surface incrementally as Phase H, I, J re-open against the cell-native input. No `rm` operations. No breaking edits to Phase D/G artefacts. All prior versions preserved per `dev/sdd-kit-2/AGENTS.md` hard rule 12.

The runtime is not being strangled. It is the segment-invariant core the pivot preserves.

## The full record

This section names the pivot's shape at the direction-doc altitude. The full audit record — Context, Decision, Consequences, Superseded artefacts, phase-by-phase status — lives in [`dev/adrs/ADR-001-cell-native-pivot.md`](../../dev/adrs/ADR-001-cell-native-pivot.md) (Accepted 2026-09-25) and [`docs/phases/pivot-status.md`](../../docs/phases/pivot-status.md).
