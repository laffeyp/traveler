# Cell-Native Direction Change v0.8

## Direction baseline with v0.7 grounding-pass closures and the field-munitions cell added

Written 2026-09-05.

This document supersedes `cell-native-direction-change-v0.7.md` (grounding pass) and `cell-native-direction-change-v0.6.md` (prior candidate baseline).
...
| v0.8 (this) | baseline | Closes each v0.7 fatal at source: §15 row rewritten to cite shipped `MachineAdapter`; §5 names `cell_alias` as its own field distinct from `factory_node`, with the five record homes named; this §0 fold-table stands in for the missing v0.4/v0.5 files. Adds a new §12.8 environment: distributed field munitions production and repair cell. Otherwise identical to v0.6. |
...
The scoping field is named `cell_alias`. It is distinct from the shipped access dimension `factory_node`, which is an access-visibility field on `AccessDecision` and its neighbours. Access scope and work scope are different concepts; conflating them via one field breaks the visibility discipline the access boundary earned in Phase C.

Homes for `cell_alias`, in v0.8:
...
| Machine | shipped record at `contracts/records.yaml:28` | Already registered |
| MachineEvidenceRecord | shipped record at `contracts/records.yaml:30` | Use shipped name |
| MachineAdapter | shipped record at `contracts/records.yaml:29` | Already registered |
