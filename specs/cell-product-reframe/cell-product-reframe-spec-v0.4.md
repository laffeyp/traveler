# Cell Product Reframe Specification v0.4

## G2 meta-spec with v0.3 grounding-pass closures

Written 2026-09-05.

This document supersedes `cell-product-reframe-spec-v0.3.md` (grounding pass) and `cell-product-reframe-spec-v0.2.md` (prior candidate baseline).
...
Companion input: `../cell-native-direction-change/cell-native-direction-change-v0.8.md`.
...
| v0.4 (this) | baseline | Closes every v0.3 fatal at source: §14 and §15 renumbered clean; §2 cites direction v0.8; §3 output list drops the reframe spec (eight outputs, count matches); §7 marks `MachineBoard` and `InventoryBoard` read paths TBD unless a shipped read path is named; §8 adds the enforcement mechanism ("every Phase D/G surface must appear in `old-ui-demotion-map.md`; any unclassified surface fails G2 close"); §10 adds citation discipline ("shipped shape must cite registry anchor or file/line"); §2 also names the reframe spec's own review-pass discipline as the standard Stage 1 arc. Preserves the full-UI-redesign-from-scratch rule and the Phase-H-blocked-until-G2 rule. |
...
Cell-Native Direction Change v0.8  (../cell-native-direction-change/cell-native-direction-change-v0.8.md)
...
The reframe spec itself moves from candidate to shipping baseline through the standard Stage 1 review-pass arc named in `dev/process-notes/phase-opening-pattern.md`. This document (v0.4) is the current candidate baseline. A v0.5 grounding pass audits v0.4; if v0.5 lands without a new fatal, v0.4 becomes the shipping baseline and G2's Stage 2 plan opens. Every review-pass revision stays on disk under this directory per the phase-opening convention.
...
| PartBuildOrPick | overlaps `InventoryItem` filtered read + `readProjectionAsCaller('OperatorHome', …)` shape |
...
| InstallEvidence | reuse of `InstallInventory` + optional `presentation_id` (Phase E) read |
| RepairCellReleaseGate | TBD in G2 (waits on RepairCellReleaseDecision shape) |
| EvidenceTrace | reuse of `SerialHistoryView` shape at cell scope |
| BlockedWork | reuse of `BlockersForRun` projection shape at cell scope |
...
Required concepts to classify (same 25 as v0.2 plus MachineAdapter per direction v0.8 §15):
