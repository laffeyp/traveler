# Physical Presence UI Overlay Specification v0.8

## Phase G specification — closes the v0.7 grounding-pass findings

Written 2026-08-28. Supersedes `ui-overlay-spec-v0.6.md`. The v0.7 pass verified every v0.5 item as closed and named three fatal citation drifts plus one small footprint note. v0.8 closes each at source. The load-bearing lesson v0.7 recorded: line-reference citations drift the next time a handler splits; function-name anchors survive. v0.8 replaces every line range v0.7 flagged with function-name cites. Every mechanism claim in this document traces to a specific file in `src/`, `contracts/`, `canvas/`, or `scenarios/`.
...
For every changed artboard, the sprint that owns it records the row that forced the change. The v0.5 pass's `citation-drift-audit` rule applies: cite by function name or by explicit fixture path, not by numeric line reference. v0.7 caught three v0.6 sites that broke the rule; v0.8 rewrote each to a function-name anchor.
...
- Wrong-item refusal state citing `wrong_item`, thrown from `BindPresentedItemToRunStep`'s expected-child check when the presented item's identity does not match the step's expected child (function name in `src/driver/handlers.ts`). A second `wrong_item` throw sits in `InstallInventory`'s bound-item check for the install-time surface (rendered on `InstallInventoryView` per §8.2), not on this screen.
...
```text
this spec (already at specs/physical-presence-ui-overlay/ui-overlay-spec-v0.8.md)
...
The v0.7 pass's four items close here: §2.2's core-truth-chain range gave way to function-name anchors; §8.2's fabricated positional signature gave way to the input-object field list plus the `InstallInventory(world, input)` handler-head fact; §8.4's wrong `wrong_item` line cite gave way to a `BindPresentedItemToRunStep` anchor with an `InstallInventory` note for the install-time surface; §6's handoff-E footprint now names `canvas/factory-ui-canvas.html` as a build-artefact mirror of the two source screens. No line reference remains in this spec that points at a moving target inside `handlers.ts`.
