...
Per-layer decisions in the dev/sdd-kit-2 eleven-layer grammar (`dev/sdd-kit-2/grammar/PRINCIPLES.md`). Every entry names what was considered, what was rejected, and where an open decision waits.
...
Open: B-Q-74/75/76/77 candidate answers applied here; the deciding sprints may promote any of the three rejected records if the sprint's design shows the field cannot carry the load.
...
- `REPORT_REGENERATION_REQUIRED` — the cascade event. Existing behaviour read `report.regeneration_required` inline; this event makes the transition auditable.
...
Access policy amendments carry `effective_at`; the freshness cascade fires on reports with `generated_at < effective_at` whose scope overlaps the amendment. This is already the existing behaviour for the single `access_policy_change_for_controlled_export` trigger in VF-012; sprint 050 generalizes.
...
The `access` module (already registered) owns everything in this pack. `EvaluateAccess` remains its sole read-decision handler; four new operations (`OpenSupportSession`, `CloseSupportSession`, `AccessAttachment`, `AmendAccessPolicy`) attach to the same module.
...
Uses the base taxonomy in `dev/sdd-kit-2/grammar/PRINCIPLES.md`. Four B-Q entries (B-Q-74/75/76/77) apply the eight-type proposal shape; each is `NEW_TAG_PROPOSED` at heart (or `PAYLOAD_FIELD_PROPOSED` for the caller-context fields). No new proposal type needed.

## What is deliberately deferred

- Retries plus dead-letter for `AmendAccessPolicy` cascade emission. §13 does not name magnitudes; a retry schedule invented here would violate the no-invention rule. Recorded, unbuilt, to be raised when a scenario needs it.
- A `SupportSessionExpiredEvent` — held open pending sprint 041's choice between state-transition expiry and predicate expiry.
- A `Customer` or `Order` record. Not required by the boundary spec; current customer scoping lives on existing records.
