...
describe("affected-population closure (persona gap 4)", () => {
  it("run close is blocked while a batch-mate serial is unremediated, and passes once it is", () => {
    ...
    w.create("AffectedPopulation", "ap", "created", { scope_type: "specific_serials", serials: ["VB-001", "VB-002"], nonconformance: "nc_a" });

    const r1 = d.executeOperation("RunCloseCheck", { run_alias: "run_a", run_close_check_alias: "rcc1" }, "quality_engineer", "s1");
    ...
    const r2 = d.executeOperation("RunCloseCheck", { run_alias: "run_a", run_close_check_alias: "rcc2" }, "quality_engineer", "s2");
  ...
  it("a same-serial closed NC of a DIFFERENT part does not count as remediation (serial reuse across parts)", () => {
    ...
    w.create("InventoryItem", "vb1", "available", { serial_number: "VB-001", part_revision: "P-A" });
    const runA = w.create("Run", "run_a", "close_check", { target_inventory: "vb1" });
    w.create("Nonconformance", "nc_a", "closed", { run: runA.id });
    w.create("AffectedPopulation", "ap", "created", { scope_type: "specific_serials", serials: ["VB-001", "VB-002"], nonconformance: "nc_a" });
    // A DIFFERENT part (P-B) happens to have a unit whose serial is also "VB-002", with its own closed NC.
    w.create("InventoryItem", "other", "available", { serial_number: "VB-002", part_revision: "P-B" });
    const runB = w.create("Run", "run_b", "closed", { target_inventory: "other" });
    w.create("Nonconformance", "nc_b", "closed", { run: runB.id });

    d.executeOperation("RunCloseCheck", { run_alias: "run_a", run_close_check_alias: "rcc" }, "quality_engineer", "s1");
    ...
    expect((d.readRecord("rcc").fields.blockers as string[]).some((b) => b.includes("VB-002"))).toBe(true);
  });
  ...
  it("a single-serial batch (the run's own serial) does not block — the run's own remediation covers it", () => {
    ...
    const runA = w.create("Run", "run_a", "close_check", { target_inventory: "vb1" });
    w.create("Nonconformance", "nc_a", "closed", { run: runA.id });
    w.create("AffectedPopulation", "ap", "created", { scope_type: "specific_serials", serials: ["VB-001"], nonconformance: "nc_a" });
    d.executeOperation("RunCloseCheck", { run_alias: "run_a", run_close_check_alias: "rcc" }, "quality_engineer", "s1");
