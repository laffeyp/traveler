...
function rfeFailures(driver: any, target: any, expected: any): number {
  return evaluateAssertions({ compiled_assertions: [{ assertion_id: "t", assertion_type: "report_field_equals", target, expected }] }, driver, new Map(), new Map()).failures.length;
}
...
describe("report supersession + report_field_equals (VF-012)", () => {
  ...
  it("supersede + regenerate, not overwrite: prior superseded (frozen S0), new generated (S1), both exist", () => {
    ...
    expect(oldR.fields.sections.access_policy_snapshot.policy_alias).toBe("customer_summary_access"); // frozen at T0 scope
    expect(newR.fields.sections.access_policy_snapshot.policy_alias).toBe("customer_extended_access"); // new scope
    // The contrast IS the teeth — impossible if the snapshot were a hardcoded constant.
    expect(oldR.fields.sections.access_policy_snapshot.policy_alias)
      .not.toBe(newR.fields.sections.access_policy_snapshot.policy_alias);
  ...
  it("report_field_equals discriminates: wrong value fails, missing path fails, non-report target fails", () => {
    ...
    expect(rfeFailures(d, { alias: "run_close_report_t0", path: PATH }, { value: "customer_summary_access" })).toBe(0); // correct
    expect(rfeFailures(d, { alias: "run_close_report_t0", path: PATH }, { value: "customer_extended_access" })).toBe(1); // wrong (old is frozen at S0)
    expect(rfeFailures(d, { alias: "run_close_report_t0", path: "sections.nope.x" }, { value: "y" })).toBe(1);          // missing path
  ...
  it("a superseded report is TERMINAL — it cannot be superseded again (superseded -> * forbidden)", () => {
    ...
    const again = vf012.driver.executeOperation("SupersedeReport", { report_alias: "run_close_report_t0", trigger: "access_policy_change_for_controlled_export" }, "worker", "sX", "k-again");
