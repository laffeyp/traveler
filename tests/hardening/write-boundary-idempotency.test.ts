...
const invCount = (d: any) => [...d.world.records.values()].filter((r: any) => r.record_type === "InventoryItem").length;
...
describe("write-boundary idempotency (transactional_unique_constraint)", () => {
  it("a duplicate key on CreateInventoryItem conflicts and creates zero new facts", () => {
    ...
    const r1 = d.executeOperation("CreateInventoryItem", { inventory_alias: "a1", serial_number: "S1", part_revision: "p" }, "planner", "s1", "K1");
    const r2 = d.executeOperation("CreateInventoryItem", { inventory_alias: "a2", serial_number: "S1", part_revision: "p" }, "planner", "s2", "K1"); // same key
  ...
  it("distinct keys are NOT collapsed — a different key writes a new record", () => {
    const d = new InMemoryProductDriver();
    d.executeOperation("CreateInventoryItem", { inventory_alias: "a1", serial_number: "S1", part_revision: "p" }, "planner", "s1", "K1");
    const r2 = d.executeOperation("CreateInventoryItem", { inventory_alias: "a2", serial_number: "S2", part_revision: "p" }, "planner", "s2", "K2"); // different key
  ...
  it("two different ops sharing a key string do NOT collide (keys are op-scoped)", () => {
    const d = new InMemoryProductDriver();
    const a = d.executeOperation("CreateInventoryItem", { inventory_alias: "inv", serial_number: "S1", part_revision: "p" }, "planner", "s1", "SHARED");
    // A DIFFERENT transactional op with the SAME key string must still succeed (not conflict).
    const b = d.executeOperation("CreateGrammarGap", { grammar_gap_alias: "g", reason: "r", gap_type: "t" }, "worker", "s2", "SHARED");
  ...
  it("a transient failure does not poison the key (retry can succeed)", () => {
    ...
    const bad = d.executeOperation("NormalizeMachineEvidence", { evidence_alias: "does-not-exist" }, "worker", "s1", "RK");
    ...
    d.executeOperation("ReceiveMachineEvidence", { alias: "e", machine_alias: "m", adapter_alias: "a", payload_type: "torque_trace", payload: { serial_number: "S", measured_torque_nm: 11.1 } }, "adapter", "s2", "RK2");
    const good = d.executeOperation("NormalizeMachineEvidence", { evidence_alias: "e" }, "worker", "s3", "RK");
  ...
  it("the write-boundary constraint is DISTINCT from the required_idempotency_key memo (different failure mode)", () => {
    ...
    const d = new InMemoryProductDriver();
    const a = d.executeOperation("ReceiveMachineEvidence", { alias: "e", machine_alias: "m", adapter_alias: "ad", payload_type: "torque_trace", payload: {} }, "adapter", "s1", "MK");
    const b = d.executeOperation("ReceiveMachineEvidence", { alias: "e2", machine_alias: "m", adapter_alias: "ad", payload_type: "torque_trace", payload: {} }, "adapter", "s2", "MK");
    expect(a.succeeded).toBe(true);
    ...
    const c = d.executeOperation("CreateInventoryItem", { inventory_alias: "x", serial_number: "S9", part_revision: "p" }, "planner", "s3", "TK");
    const e = d.executeOperation("CreateInventoryItem", { inventory_alias: "x2", serial_number: "S9", part_revision: "p" }, "planner", "s4", "TK");
