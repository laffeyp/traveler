...
function verify(d: any, lot: string, type: string, asOf?: string) {
  return d.executeOperation("VerifyCertificate", { serial_or_lot: lot, cert_type: type, as_of: asOf }, "planner", "s").output;
}
...
describe("typed supplier certificates (persona gap 8)", () => {
  it("captures a typed cert tied to a lot and verifies presence + type", () => {
    ...
    expect(verify(d, "LOT-1", "mill_cert").valid).toBe(false); // none yet
    expect(verify(d, "LOT-1", "mill_cert").reason).toBe("no_certificate");

    d.executeOperation("CaptureCertificate", { certificate_alias: "cert1", cert_type: "mill_cert", serial_or_lot: "LOT-1", cage_code: "1ABC1", expires_at: "2027-01-01T00:00:00Z" }, "planner", "s");
    const c = d.readRecord("cert1");
    expect(c.fields.cert_type).toBe("mill_cert");       // typed, not a loose attachment
    expect(c.fields.serial_or_lot).toBe("LOT-1");       // tied to the lot
    expect(c.fields.cage_code).toBe("1ABC1");

    expect(verify(d, "LOT-1", "mill_cert").valid).toBe(true);
    expect(verify(d, "LOT-1", "cofc").valid).toBe(false); // wrong type -> not satisfied
  });

  it("an expired cert does not satisfy verification", () => {
    const d = new InMemoryProductDriver();
    d.executeOperation("CaptureCertificate", { certificate_alias: "cert2", cert_type: "cofc", serial_or_lot: "LOT-2", expires_at: "2020-01-01T00:00:00Z" }, "planner", "s");
    const v = verify(d, "LOT-2", "cofc", "2026-07-01T00:00:00Z");
  ...
  it("dates compare chronologically, not lexically; a missing expiry fails closed", () => {
    ...
    d.executeOperation("CaptureCertificate", { certificate_alias: "clex", cert_type: "cofc", serial_or_lot: "LOT-L", expires_at: "2026-9-1" }, "planner", "s");
    expect(verify(d, "LOT-L", "cofc", "2026-10-01T00:00:00Z").valid).toBe(false);
    // A cert with no expiry cannot be verified -> not valid (fail closed).
    d.executeOperation("CaptureCertificate", { certificate_alias: "cnoexp", cert_type: "cofc", serial_or_lot: "LOT-N" }, "planner", "s");
    const v = verify(d, "LOT-N", "cofc", "2026-07-01T00:00:00Z");
    expect(v.valid).toBe(false);
