...
function redlineUnderReview(d: any, author: string) {
  op(d, "CreateRedlineDraft", { redline_alias: "rl", run_alias: "run_x", procedure_version_alias: "pv_x" }, author);
...
function ncAtVerificationPending(d: any, performer: string) {
  ...
  op(d, "OpenNonconformance", { nonconformance_alias: "nc", source_measurement_alias: "meas_x", run_alias: "run_x" }, "quality_1");
  op(d, "DefineAffectedPopulation", { nonconformance_alias: "nc", affected_population_alias: "ap", scope_type: "specific_serials", serials: ["VB-1"] }, "quality_1");
  op(d, "StartQualityContainment", { nonconformance_alias: "nc", containment_action_alias: "ca" }, "quality_1");
  op(d, "ActivateQualityContainment", { nonconformance_alias: "nc", containment_action_alias: "ca" }, "quality_1");
  ...
  op(d, "CompleteRework", { nonconformance_alias: "nc", rework_run_alias: "rr", rework_result: "completed" }, performer);
}
...
describe("segregation of duties (persona gap 1)", () => {
  ...
  it("approval: the author cannot approve their own redline; a distinct approver can", () => {
    ...
    const self = op(d, "RecordApprovalDecision", { approval_request_alias: "ar", approval_decision_alias: "ad1", redline_alias: "rl", decision: "approved" }, "alice");
    ...
    const other = op(d, "RecordApprovalDecision", { approval_request_alias: "ar", approval_decision_alias: "ad2", redline_alias: "rl", decision: "approved" }, "bob");
  ...
  it("verification: the person who did the rework cannot verify it; a distinct verifier can", () => {
    ...
    const self = op(d, "VerifyRework", { nonconformance_alias: "nc", verification_alias: "v1" }, "quality_1"); // reworker
    expect(self.succeeded).toBe(false);
    expect(self.failureClass).toBe("segregation_of_duties_violation");
    expect(d.readRecord("nc").state).toBe("verification_pending"); // refused, unchanged
    const other = op(d, "VerifyRework", { nonconformance_alias: "nc", verification_alias: "v2" }, "quality_2"); // distinct
    expect(other.succeeded).toBe(true);
  ...
  it("fails CLOSED: an approval carrying no approver identity is refused, not committed unsigned", () => {
    ...
    const noActor = d.executeOperation("RecordApprovalDecision", { approval_request_alias: "ar", approval_decision_alias: "ad", redline_alias: "rl", decision: "approved" }, "quality_engineer", "s1");
