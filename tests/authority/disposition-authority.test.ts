...
function ncAtDispositionPending() {
  ...
  op(d, "OpenNonconformance", { nonconformance_alias: "nc", source_measurement_alias: "meas_x", run_alias: "run_x" });
  op(d, "DefineAffectedPopulation", { nonconformance_alias: "nc", affected_population_alias: "ap", scope_type: "specific_serials", serials: ["VB-1"] });
  op(d, "StartQualityContainment", { nonconformance_alias: "nc", containment_action_alias: "ca" });
  op(d, "ActivateQualityContainment", { nonconformance_alias: "nc", containment_action_alias: "ca" });
  return d;
}
...
describe("disposition kinds + authority (persona gap 3)", () => {
  it("rejects an unregistered disposition kind", () => {
    ...
    const r = op(d, "RecordDisposition", { nonconformance_alias: "nc", disposition: "make_it_fine" });
  ...
  it("use-as-is needs quality/engineering authority: refused for an operator, allowed for quality", () => {
    ...
    const refused = op(asOperator, "RecordDisposition", { nonconformance_alias: "nc", disposition: "use_as_is" }, "operator");
    ...
    const ok = op(asQuality, "RecordDisposition", { nonconformance_alias: "nc", disposition: "use_as_is" }, "quality_engineer");
  ...
  it("repair needs authority too: refused for a planner, allowed for a manufacturing engineer", () => {
    ...
    expect(op(asPlanner, "RecordDisposition", { nonconformance_alias: "nc", disposition: "repair" }, "planner").failureClass).toBe("disposition_authority_violation");

    const asEng = ncAtDispositionPending();
    expect(op(asEng, "RecordDisposition", { nonconformance_alias: "nc", disposition: "repair" }, "manufacturing_engineer").succeeded).toBe(true);
  });

  it("fails CLOSED: use-as-is with NO role (or an empty role) is refused, not waved through", () => {
    for (const role of ["", undefined as any]) {
      const d = ncAtDispositionPending();
      const r = d.executeOperation("RecordDisposition", { nonconformance_alias: "nc", disposition: "use_as_is" }, role, "s", undefined, "someone");
      expect(r.succeeded).toBe(false);
  ...
  it("org-level kinds (rework, scrap, return_to_supplier) do not need elevated authority", () => {
    const d = ncAtDispositionPending();
    expect(op(d, "RecordDisposition", { nonconformance_alias: "nc", disposition: "rework" }, "operator").succeeded).toBe(true);
