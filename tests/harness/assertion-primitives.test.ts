...
import { runScenarioWithDriver, evaluateAssertions, runIdempotencyReplay } from "../../src/harness/run.ts";

const { driver } = runScenarioWithDriver("VF-003");
const ev = (a: any) => evaluateAssertions({ compiled_assertions: [a] }, driver, new Map(), new Map());

describe("SDD assertion primitives discriminate", () => {
  it("event_payload_contains: right foreign key passes, wrong one fails", () => {
    expect(ev({ assertion_type: "event_payload_contains", target: { event_type: "MEASUREMENT_FAILED" }, expected: { measurement_id: "measurement_torque_failed" } }).failures).toEqual([]);
    expect(ev({ assertion_type: "event_payload_contains", target: { event_type: "MEASUREMENT_FAILED" }, expected: { measurement_id: "gasket_001" } }).failures.length).toBe(1);
  });

  it("operation_output_contains: matching output passes; wrong value / missing step / empty expected fail", () => {
    const sr = new Map<string, any>([["s1", { operationName: "GetReport", succeeded: true, output: { regeneration_required: true, regeneration_reason: "reconciliation_resolution_affecting_run" } }]]);
    const oc = (target: any, expected: any) => evaluateAssertions({ compiled_assertions: [{ assertion_id: "t", assertion_type: "operation_output_contains", target, expected }] }, driver, sr, new Map()).failures.length;
    expect(oc({ step_id: "s1" }, { regeneration_required: true, regeneration_reason: "reconciliation_resolution_affecting_run" })).toBe(0); // correct
  ...
  it("event_sequence_matches: correct order passes, reversed order fails", () => {
    expect(ev({ assertion_type: "event_sequence_matches", target: {}, expected: { sequence: ["RUN_CREATED", "MEASUREMENT_FAILED", "RUN_CLOSED"] } }).failures).toEqual([]);
    expect(ev({ assertion_type: "event_sequence_matches", target: {}, expected: { sequence: ["RUN_CLOSED", "MEASUREMENT_FAILED", "RUN_CREATED"] } }).failures.length).toBe(1);
    // an empty expected sequence is a configuration error, not a vacuous pass
    expect(ev({ assertion_type: "event_sequence_matches", target: {}, expected: { sequence: [] } }).failures.length).toBe(1);
  });
  ...
  it("idempotent_replay is NOT a tautology: a COLD key re-runs the handler and the check CATCHES the duplicate", () => {
    ...
    const rme = (alias: string) => ({ alias, machine_alias: "m", adapter_alias: "a", payload_type: "torque_trace", occurred_at: "t", received_at: "t", payload: { serial_number: "S9" } });
    ...
    const warm = runIdempotencyReplay(d, { steps: [{ step_id: "s1", operation: "ReceiveMachineEvidence", input: rme("x2") }], idempotency_replay_checks: [{ check_id: "c1", step_id: "s1", idempotency_key: "warm-key" }] });
    expect(warm).toEqual([]);
    ...
    const cold = runIdempotencyReplay(d, { steps: [{ step_id: "s1", operation: "ReceiveMachineEvidence", input: rme("x3") }], idempotency_replay_checks: [{ check_id: "c2", step_id: "s1", idempotency_key: "cold-key-unused" }] });
