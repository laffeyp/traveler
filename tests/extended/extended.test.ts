...
describe("extended adversarial scenarios (in-memory)", () => {
  ...
  it("VF-011: the duplicate delivery dedups — exactly one receipt, one record, one normalize", () => {
    const ev = vf011.driver.readEventTrace().filter((e: any) => e.type === "MACHINE_EVIDENCE_RECEIVED");
  ...
  it("VF-014: the drill-down both filters AND audits (the audit event is the discriminator)", () => {
    ...
    const audit = vf014.driver.readEventTrace().find((e: any) => e.type === "BOUNDED_DRILL_DOWN_REQUESTED");
  ...
  it("VF-013: RecordApprovalDecision branches — reject -> rejected -> apply refused; approve -> approved -> apply succeeds", () => {
    const setup = (d: any) => {
      d.executeOperation("CreateProcedureVersion", { procedure_version_alias: "pv", steps: [{ alias: "st", ordinal: 1, name: "s" }] }, "eng", "b-pv");
      d.executeOperation("SubmitProcedureVersionForReview", { procedure_version_alias: "pv" }, "eng", "b-pvs");
      d.executeOperation("ReleaseProcedureVersion", { procedure_version_alias: "pv" }, "eng", "b-pvr");
      d.executeOperation("CreateRun", { run_alias: "run", run_context_snapshot_alias: "snap", procedure_version_alias: "pv", manufacturing_structure_alias: "ms", effectivity_resolution_alias: "eff", target_inventory_alias: "vb", run_step_aliases: [] }, "planner", "b-run");
      d.executeOperation("CreateRedlineDraft", { redline_alias: "rl", run_alias: "run", procedure_version_alias: "pv" }, "op", "b-rl", undefined, "author_1");
      d.executeOperation("SubmitRedline", { redline_alias: "rl" }, "op", "b-sub");
      d.executeOperation("ReviewRedline", { redline_alias: "rl" }, "eng", "b-rev");
      d.executeOperation("RequestApproval", { approval_request_alias: "ar", redline_alias: "rl" }, "eng", "b-req");
    };
    ...
    setup(dR);
    dR.executeOperation("RecordApprovalDecision", { approval_request_alias: "ar", approval_decision_alias: "ad", redline_alias: "rl", decision: "rejected" }, "eng", "r-dec", undefined, "approver_1");
    expect(dR.readRecord("rl").state).toBe("rejected");
    expect(dR.readEventTrace().map((e: any) => e.type)).toContain("REDLINE_REJECTED");
    const applyR = dR.executeOperation("ApplyRedline", { redline_alias: "rl", run_alias: "run" }, "op", "r-apply");
    ...
    setup(dA);
    dA.executeOperation("RecordApprovalDecision", { approval_request_alias: "ar", approval_decision_alias: "ad", redline_alias: "rl", decision: "approved" }, "eng", "a-dec", undefined, "approver_1");
    expect(dA.readRecord("rl").state).toBe("approved");
    expect(dA.readEventTrace().map((e: any) => e.type)).toContain("REDLINE_APPROVED");
    const applyA = dA.executeOperation("ApplyRedline", { redline_alias: "rl", run_alias: "run" }, "op", "a-apply");
