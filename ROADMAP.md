...
- **Run-close rules still unverified.** Two of the thirteen (`required_measurements_present`, `required_installations_present`) were found unevaluated by probe on 2026-07-30 and are now built; four are enforced upstream (verified by probe); two are evaluated. **Four remain untested:** `machine_evidence_reviewed_if_required` (the one probe arm that never built the rule's precondition — inconclusive, not passing), `redline_applied_before_step_complete_if_affecting_step`, `no_blocking_reconciliation_conflict`, and `run_context_snapshot_exists`. `access_policy_available` is unevaluated at the close (the read path fails closed independently). Each is one probe arm away from an answer.
- **`SkipRunStep` is registered but unimplemented**, so the skipped-step close path (B-Q-35) is unit-proven only — no bench scenario, no cross-driver diff-to-zero, no reload proof. Building the handler would let that behavior be regressed like every other (practice #12).
...
- **The demo-pack check is ungated.** `demo-packs/valve-body-assembly-v0.1/check.mjs` proves the pack invents nothing, but it is in no npm script, no gate, and no suite — so nothing turns red if the pack drifts from the registries or a rename orphans it. Whether the demo pack should be able to fail the build is a scope call (see KIT_DIARY Entry 26).
