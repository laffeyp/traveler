...
- The receiving evidence boundary specification (`specs/receiving-evidence/boundary-spec-v0.1.md`) — 15 of 15 §27 acceptance criteria pass (`acceptance/RECEIVING_ACCEPTANCE.md`).
- The access and visibility boundary specification (`specs/access-and-visibility/boundary-spec-v0.1.md`, arrived 2026-08-24) — 18 of 18 §16 acceptance criteria pass or pass-in-part (`acceptance/ACCESS_AND_VISIBILITY_ACCEPTANCE.md`).
- The UI surface design specification (`specs/ui-surface-design/ui-surface-design-spec-v0.3.md`) — 21 of 21 §25 acceptance criteria pass or pass-in-part (`docs/acceptance/UI_SURFACE_ACCEPTANCE.md`). 66 canvas artefacts under `canvas/`: 1 vocabulary reference, 2 token sheets, 8 shared components, 3 pattern libraries, 47 screen artboards (8 handheld + 39 Mac), 4 flow maps, and the handoff bundle. Published at https://claude.ai/code/artifact/347f2431-d036-4bcf-a3ad-28cc928a3dda. `docs/banner.png` rendered from `canvas/banner.dc.html` and referenced in the root README. Phase D added no code, no registry entries, and no scenario steps; every artboard cites vocabulary that existed at Phase C close.
- The Physical Presence Boundary specification (`specs/physical-presence/boundary-spec-v0.10.md`) — 33 of 33 §15 acceptance criteria pass or pass-in-part (`docs/acceptance/PHYSICAL_PRESENCE_ACCEPTANCE.md`; one row, criterion 31 `required_presentation_on_install`, is pass-in-part — the rule ships when the first scenario opts a factory node into runtime-enforced presence). Two records (`Station`, `Presentation`), six operations, seven events, one state machine, four new authorization rules plus one widened (`physical_presence` widened from `[operator, planner, quality_engineer]` to include `support_user` so VF-046 has a valid audience; the description records the widening and the §4.2 narrowing that prevents leakage; `ConsumePresentation` reuses `system_lifecycle`, no fifth rule added), 31 failure classes. `InstallInventory` extended with an optional `presentation_id`; every pre-Phase-E scenario traces byte-identical. Three driver changes: JSON-expression partial index in `backend.ts` for the one-active-per-item invariant; tuple-aware idempotency branch in `driver.ts`; deterministic `access_decision_id` on `EvaluateAccess` output. Nine new scenarios (VF-038 through VF-046) — VF-043 shipped as "adapter cannot present" rather than the plan's original "hidden identity" story, because the `physical_presence` widening made the original test succeed instead of refuse; the adapter path exercises the wrapper's authorization refusal, which is the same discipline. 19-arm coupling-mutation suite (17 arms at Phase E close plus two direct-call arms added by the 2026-08-28 review to lock chronological expiry semantics; the plan's 25 arms were consolidated because some overlapped and others were covered by scenario assertions directly). Scan contract lands as `src/harness/scan-decoder.ts` and `src/harness/scan-classifier.ts`. VF-047 was added the same day the review ran; it exercises the §12.1 non-production record-conflict branch, and the whole-bench cross-driver diff-to-zero over 47 scenarios PASSes with both drivers writing the conflicted Presentation and emitting `PRESENTATION_CONFLICT_DETECTED`. See `docs/STATE.md` §5c review-response paragraph for the four correctness fixes the review found.
...
- `acceptance/RECEIVING_ACCEPTANCE.md`, `acceptance/ACCESS_AND_VISIBILITY_ACCEPTANCE.md` — row-by-row scoring against §27 / §16.
- `notes/DEVIATION_SUMMARY.md` — quantified delta from the initial design, and where it stands.
- `notes/ADDITIONS.md` — 30 entries: every capability built on top of the original doc stack, each with its new vocabulary and its test.
...
- `notes/SESSION_2026-08-25.md` — Phase C narrative, commit ledger, gate deltas.
- `process/SDD_GENERAL_PROCESS.md` — SDD placed against the settled fields that already do what it does. Not project state; the doctrine behind it.
...
- Unify operation authorization with the §8 decision model. Row 4 of `acceptance/ACCESS_AND_VISIBILITY_ACCEPTANCE.md`'s only pass-in-part. `role_not_authorized` and `controlled_data_denied` reason codes registered as `used_by_sprint: deferred`.
