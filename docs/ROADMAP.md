...
- **Phase F — Physical Presence bench.** Input spec arrived 2026-08-28 at `specs/physical-presence-bench/bench-spec-v0.4.md`. Ran through five review passes against the shipped code (v0.5, v0.6, v0.7, v0.8) and closed at the v0.8 shipping baseline. v0.6 caught the fatal `v1:` label-prefix claim — the shipped `decodeLabel` at `src/harness/scan-decoder.ts` refuses any payload with more than three colon-separated segments, so the v0.5 grammar would have failed every generated label; v0.6 reversed to the shipped `record_type:record_alias[:checksum]` grammar with version discrimination at the label-file metadata layer. Ten runtime-touching scenarios ship as VF-048 through VF-057 (happy path, wrong item, expired, production conflict, non-production conflict via bench flow, hidden identity, manual selection, install-from-reserved refuses `state_transition_forbidden`, same-key different-tuple `idempotency_conflict`, `consuming_operation_mismatch`). Decoder-refusal tests ship as plain vitest under `tests/harness/`. Idempotency keys follow the shipped `vf-<NNN>-<call_id>` convention. The printed-label phone test runs under a local dev-tool session that loads a static thirteen-field `CallerContext` fixture with a warning banner naming this as bench scaffolding, not authentication, not the BFF — Phase H's real auth model has not landed. Sprint planning opens against v0.8. Still a test surface, not a shipped app.
