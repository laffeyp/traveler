...
| [PHASE_G_PLAN.md](PHASE_G_PLAN.md) | The Phase G task breakdown — 13 sprints (126-138), seven sub-phases, the fifteen-screen scope under four outcome classes, the two triggers evaluated at close, the F2b addendum landing alongside. |
| [phase-h-input-package.md](phase-h-input-package.md) | Phase H's derived-endpoint input: 19 (screen, action) rows across seven Phase G screens with the seven-field shape (registered op / caller context / visibility profile / idempotency / refusal envelope / source). Four `proposed` markers on names Phase H's team will settle; every other name resolves against the shipped registries today. |
| [phase-g-phase-m-trigger.md](phase-g-phase-m-trigger.md), [phase-g-handoff-a-track-2-trigger.md](phase-g-handoff-a-track-2-trigger.md), [phase-g-ij-recommendation.md](phase-g-ij-recommendation.md), [phase-g-screen-to-call-log-map.md](phase-g-screen-to-call-log-map.md), [phase-g-remaining-handoffs.md](phase-g-remaining-handoffs.md) | Phase G closeout docs: both triggers NOT FIRED; Desktop-first alpha; per-changed-artboard evidence cite; every remaining handoff-F or handoff-A track 2 marker listed with its reason. |
