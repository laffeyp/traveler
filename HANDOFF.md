...
Written 2026-08-25 at the close of Phase C. Read this first, then `STATE.md`, then `ROADMAP.md`.

## 1. What this is

A contract-first Manufacturing Execution & Record System for complex hardware. TypeScript on Node. Behavior is data: 16 locked YAML registries in `contracts/` name every operation, event, record, state machine, projection, report, run-close rule, receiving rule, authorization rule, assertion type, module, reason code, failure class, and visibility profile the system speaks. `src/` is a generic executor over the registries — the runtime does not invent behavior.

The hard rule that makes the whole thing work: **no invention.** If a required behavior is not defined by the contract stack, the executor emits a `ContractGap`, a `not_implemented`, or a recorded B-Q — never a guess. A scenario referencing an unregistered name compiles to a ContractGap. A handler emitting an unregistered event throws at the emit site and the operation rolls back. A caller_type not listed by an authorization rule refuses fail-closed with `authorization_denied`.

Two drivers sit behind one interface: `InMemoryProductDriver` and `BackendProductDriver` (node:sqlite). The whole-bench cross-driver check asserts they produce equivalent traces on 37 scenarios. Fifteen durability proofs assert a fresh-from-disk backend instance still holds the fact.

The project runs under **Signal-Driven Development kit v2** (`sdd-kit-2/`, vendored read-only). The contract registries are this project's locked vocabulary — the equivalent of `signals/0.1.json` in the kit's example.
...
No build step, no bundler. The runtime reads `.ts` directly via Node's type-stripping. Every gate above runs in seconds.
...
| `scenarios/` | 38 scenarios, 779 steps, each pure data — `scenario.yaml` + `references.yaml` per scenario. Numbering skips VF-017..023; see `WORKING_AGREEMENT.md §Numbering`. |
...
- **`STATE.md`** — where the build stands against every governing document. Read first after this handoff.
...
- **`DEVIATION_SUMMARY.md`** — quantified delta from the initial design, how, and where it stands.
...
- **`BLACKBOARD.md`** — the SDD project state board. Single-writer per section. Architect (human user) owns `## Decisions`; the Agent owns the other six sections.
- **`KIT_DIARY.md`** — 32 entries + phase syntheses recording what worked, what got in the way, and what the next kit revision should carry. Practices 0-34 accreted here; the top-level catalog lives in `sdd-kit-2/TECHNIQUES.md`.
- **`ADDENDUMS.md`** — dated project-stamped technique captures held here until they stabilize across a second project and fold into TECHNIQUES.md.
- **`WORKING_AGREEMENT.md`** — per-project overrides on top of `sdd-kit-2/AGENTS.md`: authority order, canonical home registry, cadence, non-inference rule.
- **`SESSION_2026-08-25.md`** — the Phase C day's narrative, commit ledger, and gate deltas.
- **`SDD_GENERAL_PROCESS.md`** — theory note: SDD set against the settled fields that already do what it does. Not project state; the doctrine behind it.
- **`contracts/CONTRACT_GAPS.md`** — the typed B-Q ledger, 77 entries, every underspecification and its resolution.

## 6. The rules the next person must follow

These are hard rules, not guidance. Each has a reason and a record of what happens when it is broken.

1. **The registries are the vocabulary.** Do not add a handler that emits an unregistered event. Do not add a state a state machine does not declare. Do not add a reason code that no §14 entry names. Registered names are the contract; if the code needs a name the registries do not have, halt with a B-Q and record the call. `tests/consolidation/handler-registration.test.ts` enforces the reverse direction (every handler maps to a registered op).

2. **Fail-closed on missing input.** Every guard added since sprint 010 refuses when its required input is absent, empty, malformed, or unresolvable. Absence is refusal. `contracts/reason-codes.yaml` registers the specific name each refusal carries; a generic `authorization_denied` is not enough when a specific §14 name exists. The distrust-the-green review across fifteen straight increments has never come back empty; the dominant defect shape is fail-open on ugly input.

3. **Coupling mutation, in-session, on every load-bearing guard.** Suppress the guard, observe a specific test go red, restore. This is the check the receiving-boundary mutation battery and the fail-closed battery both institutionalize; it is also what caught the vacuous audit test in the 2026-08-25 red-team pass. A green whose test cannot go red under a targeted defect proves nothing.

4. **Byte-identical is a hard claim; verify it correctly.** The cross-driver diff-to-zero measures two drivers running the same code path for equivalence. It does not prove the trace matches a prior snapshot. A change that affects both drivers identically passes it. The 2026-08-25 red-team caught the overstatement — `MUTATION_TEST: true` added to an event payload passed diff-to-zero. `event_payload_contains` subset assertions do not catch NEW fields either. A stored-golden-trace per-scenario baseline is the deferred follow-up (`ROADMAP.md §Post-Phase-C deferred items`). Until it lands, do not claim "byte-identical against a baseline" — claim "cross-driver equivalent" and "existing assertions still hold".

5. **Same-word audit at authoring.** The receiving pack proposed 13 records; the mapping-pass same-word audit reduced them to 3. The Phase C pack proposed 12 new-vocabulary items; the same audit reduced them to 1 new record (`SupportSession`) plus fields on existing records. Ask "do we already have a word for this" before adding a name. B-Q candidate answers to the four mapping calls (74/75/76/77) all kept the answers as fields rather than records.
...
## 7. What is deferred (recorded, unbuilt)
...
- **Report regeneration triggers beyond the two wired.** `report_definition_change` and `source_record_correction` are currently inert. A fully automatic daemon-style supersession has operator- and reconciliation-driven precedent but no autonomous path.
- **Bounded drill-down's "capped" + arbitrary-predicate-rejection obligations** — deferred until a cap magnitude enters the contract.
...
Recorded as choices so they read as choices, not misses:

- **Offline-first node execution** — the spec says don't; node sync is simulated on purpose.
- **eBOM / design-BOM reconciliation + formal FCA/PCA** — left to PLM.
- **Full ERP / PLM, scheduling, machine control, physical simulation** — original scope non-goals.
- **Counterfeit-part screening + source-inspection sessions** — a boundary inside supplier certs, out of scope.
...
- **Gates broken after a pull:** run `npm install` first (dev deps may have shifted). Then walk the gates in order: contracts → schemas → tsc → vitest → bench → backend. The first one to red usually points at the file that broke.
...
- **The registry validator refuses:** the message names the specific rule broken. Common causes: an event's `producer_operations` list is missing the operation that emits it (add it there); a state machine references an operation that isn't in `operations.yaml`; a caller_type in an authorization rule isn't in `modules.yaml`. The validator fails closed on every unknown; there is no permissive mode.
- **A test is green but you don't trust it:** suppress the guard the test claims to check, watch the specific test go red, restore. If the test doesn't go red under a targeted mutation, the test is decoupled from what it claims to prove. This is Addendum A's practice #3, and it caught the vacuous audit test in the 2026-08-25 red-team pass.
- **You want to add a new operation:** register it in `contracts/operations.yaml` (authorization rule, module, exposure, idempotency, observability_ref, compatibility_ref, events_emitted). Register the events it emits in `contracts/events.yaml` with the operation in `producer_operations`. If the operation drives a state transition, add it to `contracts/state-machines.yaml`. Regenerate schemas (`npm run generate:schemas`) and types (`npm run generate:types`). Add the handler in `src/driver/handlers.ts`. Add unit tests. Run `validate:contracts` — the reverse-registration check will refuse if the handler is not registered.
- **You want to add a new access dimension** (per Phase C's opt-in shape): add the caller-context field to `src/driver/visibility.ts` `CallerContext`. Thread it through `readRecordAsCaller` in `driver.ts`. Add the check to `EvaluateAccess` in `handlers.ts` — the check reads a target-side scoping field and refuses fail-closed with a specific §14 reason when the caller's context is absent, empty, or mismatched. Register the reason in `contracts/reason-codes.yaml` and `contracts/failure-classes.yaml`. Add a discrimination unit test in `tests/access/`. Verify the whole-bench cross-driver diff-to-zero still passes (existing records carry no scoping field, so the check stays dormant).

## 10. The through-line

The design's deepest claim — a factory that speaks a typed vocabulary, refuses to blur distinct states, and never asserts false certainty — is executable, holds under adversity, and holds through three governing boundaries. Across fifteen straight increments the distrust-the-green review has never come back empty by inspection; on Phase C the defects it found were smaller and lived at the ledger's edge (claims about the code rather than defects in the code), but they were real. The load-bearing thing was never the green; it was the discipline that refused to trust it. Every check names the specific reason it refuses under. Every registered name is either used, `maps_to` a name that is used, or explicitly `deferred` with a note pointing at the sprint that would use it. Nothing floats.

Read `STATE.md` next for the file-and-line detail against every governing document. Read `ROADMAP.md` for what's shipped and what's deferred. When in doubt, run the gates.
