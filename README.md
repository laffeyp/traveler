...
Someone with no manufacturing-industry experience tried to reverse-engineer a working factory-execution system from public sources alone, using an LLM and the Signal-Driven Development method. The domain — software for running distributed factories that build complex hardware — was picked because the public record is thick: standards, published architectures, job postings, open-source projects, regulatory guidance, vendor talks, conference material. None of the input specifications came from anyone under NDA. This is what came out.

The public sources the research dossier names in §9: First Resonance ION for manufacturing execution vocabulary, Hadrian for distributed-factory ambition, SpaceX for the claim that at enough complexity factory software becomes part of the production system, ISA-95 / B2MML for operations vocabulary, MTConnect and OPC UA for machine data, ERP/MRP vocabulary, AS9100 / AS13100 / NAS412 for aerospace quality, FAA production guidance, ITAR / EAR for export control. Each source evaluated on five things: domain relevance, interface quality, scale posture, adapter burden, architectural trust. Full source-evaluation lens in [`specs/founding-stack/01-research-dossier-v0.12.md`](specs/founding-stack/01-research-dossier-v0.12.md) §10.

The work ran in two directions across the same line. Forward: read the domain out of the public record, then write governing specifications a Tier-1 LLM could implement without inventing behavior — theory dossier, product spec, technical architecture, executable contract, test-oracle harness, first scenario, build plan. Reverse: for the external systems the product must talk to (ERP, PLM, operator stations, machine adapters, identity providers), build doc-derived simulators from the public docs and standards rather than real integrations. §30 of the research dossier calls the second one a doc-derived reverse harness. The hard rule both directions share: no invention.

The method underneath is Signal-Driven Development — treat a body of text as an imperfect, repeated copy of a small typed grammar, lock the grammar, use it to both build and check. Most of the operation is old work under a new name: process mining, spec mining, grammatical inference, model extraction, model-driven reverse engineering, Rules-as-Code, computer-interpretable clinical guidelines. The theory note in [`docs/SDD_GENERAL_PROCESS.md`](docs/SDD_GENERAL_PROCESS.md) sets SDD against the five fields that already do the same thing and names the parts that are actually new. There are few.
